...
{
    nlohmann::json json = {
        {"jsonrpc", "2.0"}, {"method", "getblock"}, {"params", nlohmann::json::array({"0x123", true})}, {"id", 42}};
...
TEST_F(RpcRequestTest, TestFromJsonPartial)
{
    ...
    nlohmann::json json = {{"method", "getversion"}};
...
TEST_F(RpcRequestTest, TestRoundTrip)
{
    ...
    nlohmann::json original_params = {{"hash", "0x1234567890abcdef"}, {"verbose", true}, {"index", 123}};
...
TEST_F(RpcRequestTest, TestDifferentParamTypes)
{
    ...
    nlohmann::json object_params = {{"key1", "value1"}, {"key2", 42}, {"key3", true}};
...
TEST_F(RpcRequestTest, TestCommonRpcMethods)
{
    ...
    std::vector<std::string> common_methods = {
        "getversion",     "getblockcount", "getbestblockhash", "getblock",   "getrawtransaction", "sendrawtransaction",
        "invokefunction", "invokescript",  "getcontractstate", "getstorage", "getapplicationlog"};
...
TEST_F(RpcRequestTest, TestInvalidJson)
{
    ...
    EXPECT_NO_THROW({ RpcRequest request = RpcRequest::FromJson(invalid_json); });
}
