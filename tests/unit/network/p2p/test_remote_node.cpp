...
protected:
    void SetUp() override {
        ...
        test_version_data.timestamp = std::chrono::duration_cast<std::chrono::seconds>(
        ...
        test_message_types = {
            ...
            network::p2p::MessageType::GetAddr,
            network::p2p::MessageType::Addr,
            network::p2p::MessageType::GetBlocks,
            network::p2p::MessageType::GetHeaders,
            network::p2p::MessageType::Headers,
            ...
            network::p2p::MessageType::NotFound,
            network::p2p::MessageType::Reject
        };
        
        // Node capabilities
        test_capabilities = {
            network::p2p::NodeCapability::FullNode,
            network::p2p::NodeCapability::TcpServer,
        ...
        remote_node->OnConnected += [this](const std::string& endpoint) {
            connections_established++;
        };
        
        remote_node->OnDisconnected += [this](const std::string& endpoint) {
            connections_lost++;
        };
        
        remote_node->OnHandshakeCompleted += [this](const network::p2p::VersionMessage& version) {
        ...
        remote_node->OnMessageSent += [this](const network::p2p::Message& message) {
            messages_sent++;
        };
...
TEST_F(RemoteNodeTest, GetPeerInfo) {
    if (remote_node->Connect(test_remote_endpoint)) {
        ...
        remote_node->SetPeerInfo(test_peer_info.version, test_peer_info.services, 
                                test_peer_info.user_agent, test_peer_info.start_height, test_peer_info.relay);
...
TEST_F(RemoteNodeTest, ConcurrentOperations) {
    if (remote_node->Connect(test_remote_endpoint)) {
        ...
        for (int i = 0; i < 3; ++i) {
            threads.emplace_back([this, &successful_sends, i]() {
                for (int j = 0; j < 5; ++j) {
                    auto message = CreateTestMessage(
                        test_message_types[(i * 5 + j) % test_message_types.size()],
