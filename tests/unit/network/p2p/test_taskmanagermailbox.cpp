...
class TaskManagerMailboxTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        test_peer_addresses = {"192.168.1.10:10333", "192.168.1.20:10333", "203.0.113.1:10333", "198.51.100.1:10333",
                               "10.0.0.1:10333"};
        ...
        mailbox->OnTaskCompleted += [this](const network::p2p::Task& task) { tasks_completed++; };

        mailbox->OnTaskFailed += [this](const network::p2p::Task& task, const std::string& error) { tasks_failed++; };

        mailbox->OnTaskTimeout += [this](const network::p2p::Task& task) { tasks_timeout++; };
    }
...
// Complete TaskManagerMailbox test methods - production-ready implementation matching C# UT_TaskManagerMailbox.cs
// exactly
...
TEST_F(TaskManagerMailboxTest, TaskPriorityOrdering)
{
    ...
    auto low_task =
        CreateTestTask("low_priority", network::p2p::TaskPriority::Low, network::p2p::MessageType::GetBlocks);
    auto normal_task =
        CreateTestTask("normal_priority", network::p2p::TaskPriority::Normal, network::p2p::MessageType::GetHeaders);
...
TEST_F(TaskManagerMailboxTest, CapacityLimits)
{
    ...
    {
        auto task = CreateTestTask("high_" + std::to_string(i), network::p2p::TaskPriority::High,
...
TEST_F(TaskManagerMailboxTest, GetTaskById)
{
    ...
    auto original_task =
        CreateTestTask("findable_task", network::p2p::TaskPriority::Normal, network::p2p::MessageType::Inventory);
...
TEST_F(TaskManagerMailboxTest, TaskTimeout)
{
    ...
    auto timeout_task =
        CreateTestTask("timeout_test", network::p2p::TaskPriority::Normal, network::p2p::MessageType::Ping);
    ...
    for (int i = 0; i < 5; ++i)
    {
        threads.emplace_back(
            [this, &successful_adds, i]()
            {
                for (int j = 0; j < 20; ++j)
                {
                    auto task = CreateTestTask("concurrent_" + std::to_string(i) + "_" + std::to_string(j),
                                               network::p2p::TaskPriority::Normal, network::p2p::MessageType::GetData);
...
TEST_F(TaskManagerMailboxTest, TaskRetryMechanism)
{
    ...
    auto retry_task =
        CreateTestTask("retry_test", network::p2p::TaskPriority::Normal, network::p2p::MessageType::Transaction);
...
TEST_F(TaskManagerMailboxTest, PerformanceStressTest)
{
    ...
    for (size_t i = 0; i < stress_test_task_count && i < 1000; ++i)
    {  // Limit for test performance
        auto task = CreateTestTask("stress_" + std::to_string(i),
...
TEST_F(TaskManagerMailboxTest, BatchOperations)
{
    ...
    {
        auto task = CreateTestTask("batch_" + std::to_string(i), network::p2p::TaskPriority::Normal,
...
TEST_F(TaskManagerMailboxTest, IsFull)
{
    ...
    {
        auto task = CreateTestTask("full_" + std::to_string(i), network::p2p::TaskPriority::Normal,
