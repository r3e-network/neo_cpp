...
protected:
    ...
    std::unique_ptr<ApplicationEngine> GetEngine(bool has_container = false, bool has_snapshot = false,
        ...
        auto engine = ApplicationEngine::Create(TriggerType::Application, tx, snapshot_cache, 
                                              block, protocol_settings_, gas);
    ...
    std::unique_ptr<Block> MakeBlock(Transaction* tx, const UInt256& prev_hash, uint32_t tx_count) {
        ...
        header->SetIndex(0);
        header->SetTimestamp(std::chrono::duration_cast<std::chrono::milliseconds>(
...
TEST_F(BlockAllMethodsTest, ToJson) {
    ...
    EXPECT_EQ("0x942065e93848732c2e7844061fa92d20c5d9dc0bc71d420a1ea71b3431fc21b4", 
              json_obj->GetString("hash"));
    ...
    EXPECT_EQ("0xb9bbfb2804f7582fd4340f5d87d741242afd29d3a02a5c9caa9b67325dbe236c", 
              json_obj->GetString("merkleroot"));
