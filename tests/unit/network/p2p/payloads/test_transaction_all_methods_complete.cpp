...
protected:
    ...
    std::shared_ptr<Neo3Transaction> CreateTestTransaction() {
        ...
        witness->VerificationScript = Contract::CreateSignatureContract(
...
TEST_F(TransactionAllMethodsTest, TestSerializeSize) {
    ...
    // Add witness size
    size_t witness_size = 1 + test_tx_->Witnesses[0]->InvocationScript.size() + 
                         1 + test_tx_->Witnesses[0]->VerificationScript.size();
...
TEST_F(TransactionAllMethodsTest, TestVerifyWitnesses) {
    ...
    auto engine = ApplicationEngine::Create(TriggerType::Verification, test_tx_, cloned_cache, nullptr, protocol_settings_);
