...
protected:
    void SetUp() override {
        ...
        test_message_types = {
            ...
            network::p2p::MessageType::Version,
            network::p2p::MessageType::Verack
        };
        ...
        task_session->OnTaskCompleted += [this](const network::p2p::Task& task) {
            tasks_completed++;
        };
        
        task_session->OnTaskFailed += [this](const network::p2p::Task& task, const std::string& error) {
            tasks_failed++;
        };
        
        task_session->OnMessageSent += [this](const network::p2p::Message& message) {
            messages_sent++;
        };
        
        task_session->OnMessageReceived += [this](const network::p2p::Message& message) {
            messages_received++;
        };
    ...
    std::shared_ptr<network::p2p::Message> CreateTestMessage(
        network::p2p::MessageType type,
...
TEST_F(TaskSessionTest, SessionCapacityLimits) {
    ...
    for (size_t i = 0; i < session_config->max_concurrent_tasks + 5; ++i) {
        auto task = CreateTestTask("overflow_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::GetBlocks);
...
TEST_F(TaskSessionTest, GetTaskById) {
    ...
    auto original_task = CreateTestTask("findable_task", network::p2p::TaskPriority::High, network::p2p::MessageType::GetHeaders);
...
TEST_F(TaskSessionTest, GetAllActiveTasks) {
    ...
    for (size_t i = 0; i < 5 && i < session_config->max_concurrent_tasks; ++i) {
        auto task = CreateTestTask("active_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::Block);
        if (task_session->AddTask(task)) {
            added_count++;
        }
    }
...
TEST_F(TaskSessionTest, ClearAllTasks) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        auto task = CreateTestTask("clear_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::Inventory);
...
TEST_F(TaskSessionTest, SessionStatistics) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        auto task = CreateTestTask("stats_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::GetData);
        task_session->AddTask(task);
    }
...
TEST_F(TaskSessionTest, SessionHealthCheck) {
    ...
    // Add some load
    for (size_t i = 0; i < 3; ++i) {
        auto task = CreateTestTask("health_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::Block);
        task_session->AddTask(task);
    }
...
TEST_F(TaskSessionTest, PerformanceStressTest) {
    ...
    for (size_t i = 0; i < 100; ++i) { // Limited for test performance
        auto task = CreateTestTask(
...
TEST_F(TaskSessionTest, SessionCleanup) {
    ...
    for (size_t i = 0; i < 3; ++i) {
        auto task = CreateTestTask("cleanup_" + std::to_string(i), network::p2p::TaskPriority::Normal, network::p2p::MessageType::GetHeaders);
        task_session->AddTask(task);
    }
