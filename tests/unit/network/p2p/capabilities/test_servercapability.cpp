...
class ServerCapabilityTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        // Test server types
        test_server_types = {
            network::p2p::capabilities::ServerType::TcpServer, network::p2p::capabilities::ServerType::WsServer,
            network::p2p::capabilities::ServerType::HttpServer, network::p2p::capabilities::ServerType::RpcServer};
        ...
        test_capability_data = {{"tcp_port", std::to_string(test_tcp_port)},
    ...
    std::shared_ptr<network::p2p::capabilities::ServerCapability>
    CreateTestCapability(network::p2p::capabilities::ServerType type, uint16_t port, bool enabled = true)
    {
        auto capability = std::make_shared<network::p2p::capabilities::ServerCapability>();

        std::map<std::string, std::string> config = {{"type", std::to_string(static_cast<int>(type))},
