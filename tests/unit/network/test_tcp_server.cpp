...
class TcpNetworkTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        server_->SetConnectionAcceptedCallback(
    ...
    bool WaitForServerMessages(size_t count, std::chrono::milliseconds timeout = std::chrono::seconds(5))
    {
        ...
        return serverMessageReceived_.wait_for(lock, timeout,
                                               [this, count]() { return serverMessages_.size() >= count; });
    }
    ...
    bool WaitForClientMessages(size_t count, std::chrono::milliseconds timeout = std::chrono::seconds(5))
    {
        ...
        return clientMessageReceived_.wait_for(lock, timeout,
                                               [this, count]() { return clientMessages_.size() >= count; });
    }
...
TEST_F(TcpNetworkTest, ClientConnect)
{
    ...
    // Set message handler
    clientConnection_->SetMessageReceivedCallback([this](const Message& message) { HandleClientMessage(message); });
...
TEST_F(TcpNetworkTest, ThreadSafety)
{
    ...
    for (int i = 0; i < numThreads; i++)
    {
        threads.emplace_back(
            [this, i, messagesPerThread, &sentCount]()
            {
