...
protected:
    ...
    bool VerifyPortMapping(const network::UPnP::PortMapping& mapping) {
        ...
        if (mapping.external_port == 0 || mapping.internal_port == 0) return false;
        if (mapping.internal_ip.empty()) return false;
        if (mapping.description.empty()) return false;
        if (mapping.lease_duration == 0) return false;
...
TEST_F(UPnPTest, PerformanceStressTest) {
    ...
    for (size_t i = 0; i < stress_test_port_count && i < 20; ++i) { // Limit for test performance
        auto mapping = CreateTestMapping(test_port + i, test_protocol);
