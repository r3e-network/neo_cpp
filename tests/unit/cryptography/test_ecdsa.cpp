...
TEST_F(ECDSATest, KeyGeneration)
{
    ...
    bool all_zeros =
        std::all_of(key_pair.GetPrivateKey().begin(), key_pair.GetPrivateKey().end(), [](uint8_t b) { return b == 0; });
...
TEST_F(ECDSATest, SignatureMalleabilityProtection)
{
    ...
    // s should be <= n/2
    std::vector<uint8_t> half_order = {0x7F, 0xFF, 0xFF, 0xFF, 0x80, 0x00, 0x00, 0x00, 0x7F, 0xFF, 0xFF,
...
TEST_F(ECDSATest, PerformanceCharacteristics)
{
    ...
    std::cout << "Key generation: " << (keygen_duration.count() / static_cast<double>(num_operations)) << " ms/op"
              << std::endl;
    std::cout << "Signing: " << (signing_duration.count() / static_cast<double>(num_operations)) << " ms/op"
              << std::endl;
    std::cout << "Verification: " << (verification_duration.count() / static_cast<double>(num_operations)) << " ms/op"
              << std::endl;
}
