...
TEST_F(BLS12381CompleteTest, G1PointConstruction)
{
    ...
    EXPECT_TRUE((compressed[0] & 0x80) != 0);  // Compression flag should be set
...
TEST_F(BLS12381CompleteTest, DISABLED_PerformanceBenchmarks)
{
    ...
    std::cout << "G1 scalar multiplication: " << duration.count() / iterations << " µs per operation" << std::endl;
    ...
    std::cout << "Pairing: " << duration.count() / iterations << " µs per operation" << std::endl;
    ...
    std::cout << "Signature verification: " << duration.count() / iterations << " µs per operation" << std::endl;
}
