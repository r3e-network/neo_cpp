  ...
  protected:
    void SetUp() override
    {
        ...
        test_point1 = std::make_shared<cryptography::ecc::ECPoint>(
            cryptography::ecc::ECFieldElement(cryptography::BigInteger::FromHexString(
        ...
        compressed_point_data =
            io::ByteVector::Parse("0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798");
        uncompressed_point_data =
            io::ByteVector::Parse("0479be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798483ada7726a3c4655d"
                                  "a4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8");
        ...
        small_scalars = {cryptography::BigInteger::Zero(),         cryptography::BigInteger::One(),
                         cryptography::BigInteger::FromInt64(2),   cryptography::BigInteger::FromInt64(3),
                         cryptography::BigInteger::FromInt64(10),  cryptography::BigInteger::FromInt64(255),
                         cryptography::BigInteger::FromInt64(256), cryptography::BigInteger::FromInt64(65537)};

        // Large scalars near curve order for edge case testing
        curve_order =
            cryptography::BigInteger::FromHexString("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141");
        large_scalars = {curve_order - cryptography::BigInteger::One(),
...
TEST_F(ECPointTest, GeneratorPointIsValid)
{
    ...
    auto expected_x = cryptography::ecc::ECFieldElement(cryptography::BigInteger::FromHexString(
        "55066263022277343669578718895168534326250603453777594175500187360389116729240"));
    auto expected_y = cryptography::ecc::ECFieldElement(cryptography::BigInteger::FromHexString(
        "32670510020758816978083085130507043184471273380659243275938904335757337482424"));
...
TEST_F(ECPointTest, InvalidPointDeserialization)
{
    ...
    EXPECT_THROW(cryptography::ecc::ECPoint::FromCompressedBytes(invalid_point_data), std::invalid_argument);

    // Test with wrong size data
    io::ByteVector wrong_size_data =
        io::ByteVector::Parse("0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f817");
    EXPECT_THROW(cryptography::ecc::ECPoint::FromCompressedBytes(wrong_size_data), std::invalid_argument);
}
...
TEST_F(ECPointTest, EqualityAndComparison)
{
    ...
    auto same_point = std::make_shared<cryptography::ecc::ECPoint>(test_point1->GetX(), test_point1->GetY());
