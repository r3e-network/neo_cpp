...
protected:
    void SetUp() override {
        ...
        test_private_key = cryptography::BigInteger::FromHexString("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
        known_keypair = std::make_shared<cryptography::ecc::KeyPair>(test_private_key);
        
        // Another test key pair for comparison
        test_private_key2 = cryptography::BigInteger::FromHexString("d6e28da05f62e00be5aa477af5040696f24b2d996e22d9ec0e8fede8d9d6e2a7");
        ...
        invalid_private_keys = {
            ...
            cryptography::BigInteger::FromHexString("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364141"), // Curve order (invalid)
            cryptography::BigInteger::FromHexString("fffffffffffffffffffffffffffffffebaaedce6af48a03bbfd25e8cd0364142"), // > Curve order
        };
    ...
    bool VerifySignature(const cryptography::ecc::KeyPair& keypair, const io::ByteVector& message, const io::ByteVector& signature) {
        try {
    ...
    std::shared_ptr<cryptography::ecc::KeyPair> CreateTestKeyPair(uint64_t seed) {
        ...
        while (seeded_key >= cryptography::ecc::KeyPair::GetCurveOrder() || seeded_key == cryptography::BigInteger::Zero()) {
...
TEST_F(KeyPairTest, InvalidPrivateKeysThrowException) {
    for (const auto& invalid_key : invalid_private_keys) {
        EXPECT_THROW(
            cryptography::ecc::KeyPair(invalid_key),
...
TEST_F(KeyPairTest, SignatureVerificationWithDifferentKeyPairs) {
    ...
    for (size_t i = 0; i < test_keypairs.size(); ++i) {
        for (size_t j = 0; j < test_keypairs.size(); ++j) {
            ...
            EXPECT_EQ(should_verify, does_verify) 
                << "Signature verification mismatch for keypair " << i << " vs " << j;
        }
    }
}
