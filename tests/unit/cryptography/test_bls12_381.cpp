...
TEST(BLS12_381Test, AggregateSignature)
{
    ...
    bool valid =
        VerifyAggregateSignature({publicKey1, publicKey2}, {message1.AsSpan(), message2.AsSpan()}, aggregateSignature);
    EXPECT_TRUE(valid);

    // Verify with different messages
    bool valid2 =
        VerifyAggregateSignature({publicKey1, publicKey2}, {message2.AsSpan(), message1.AsSpan()}, aggregateSignature);
    EXPECT_FALSE(valid2);

    // Verify with different public keys
    bool valid3 =
        VerifyAggregateSignature({publicKey2, publicKey1}, {message1.AsSpan(), message2.AsSpan()}, aggregateSignature);
    EXPECT_FALSE(valid3);

    // Verify with different signature
    G1Point aggregateSignature2 = AggregateSignatures({signature2, signature1});
    bool valid4 =
        VerifyAggregateSignature({publicKey1, publicKey2}, {message1.AsSpan(), message2.AsSpan()}, aggregateSignature2);
    EXPECT_FALSE(valid4);
