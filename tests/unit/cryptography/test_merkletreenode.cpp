...
protected:
    void SetUp() override {
        ...
        known_tree_hashes = {
            io::UInt256::Parse("deadbeefcafebabedeadbeefcafebabedeadbeefcafebabedeadbeefcafebabe"),
    ...
    std::shared_ptr<cryptography::MerkleTreeNode> BuildMerkleTree(
        std::vector<std::shared_ptr<cryptography::MerkleTreeNode>>& nodes) {
    ...
    io::UInt256 CalculateExpectedRoot(const std::vector<io::UInt256>& hashes) {
        if (hashes.empty()) return io::UInt256::Zero();
    ...
    bool VerifyTreeIntegrity(std::shared_ptr<cryptography::MerkleTreeNode> node) {
        ...
        if (!node->GetLeftChild()) return false;
...
TEST_F(MerkleTreeNodeTest, MerkleProofGeneration) {
    ...
    for (size_t i = 0; i < leaf_nodes.size(); ++i) {
        ...
        // Verify proof
        bool proof_valid = cryptography::MerkleTreeNode::VerifyMerkleProof(
            leaf_nodes[i]->GetHash(), proof, tree_root->GetHash());
...
TEST_F(MerkleTreeNodeTest, MerkleProofVerification) {
    ...
    bool is_valid = cryptography::MerkleTreeNode::VerifyMerkleProof(
        proof_hashes[0], proof, proof_root->GetHash());
    
    EXPECT_TRUE(is_valid);
    
    // Invalid proof should fail
    bool invalid_proof = cryptography::MerkleTreeNode::VerifyMerkleProof(
        proof_hashes[1], proof, proof_root->GetHash());
