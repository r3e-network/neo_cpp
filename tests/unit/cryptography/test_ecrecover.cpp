...
{
    // Test with real secp256k1 test vectors from Ethereum ecosystem
    // Message: "Hello World" hashed with Keccak256
    ByteVector messageHash = ByteVector::Parse("ce0677bb30baa8cf067c88db9811f4333d131bf8bcf12fe7065d211dce971008");
    ByteVector signature = ByteVector::Parse("90f27b8b488db00b00606796d2987f6a5f59ae62ea05effe84fef5b8b0e549984a691139a"
                                             "d57a3f0b906637673aa2f63d1f55cb1a69199d4009eea23ceaddc93");
...
TEST_F(ECRecoverTest, TestKnownVectors)
{
    ...
    ByteVector signature = ByteVector::Parse("1b17e8c4c83a3f4b6a5d9ce8f3d3e8f7c8d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8"
...
TEST_F(ECRecoverTest, TestInvalidInputs)
{
    ...
    // Test with invalid recovery ID
    {
        ...
        EXPECT_THROW(ECRecover(messageHash.AsSpan(), validSig.AsSpan(), static_cast<uint8_t>(-1)),
...
TEST_F(ECRecoverTest, TestRecoveryConsistency)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        try
        {
            ...
            {
                // Verify consistency
                EXPECT_EQ(firstResult.value(), recovered.value())
                    << "Recovery produced different results on iteration " << i;
            }
        }
        catch (const std::exception& e)
        {
...
TEST_F(ECRecoverTest, TestDifferentMessagesProduceDifferentResults)
{
    ...
    TestCase cases[] = {{"ce0677bb30baa8cf067c88db9811f4333d131bf8bcf12fe7065d211dce971008",
                         "90f27b8b488db00b00606796d2987f6a5f59ae62ea05effe84fef5b8b0e549984a691139ad57a3f0b906637673aa2"
                         "f63d1f55cb1a69199d4009eea23ceaddc93",
                         1},
                        {"d25688cf0ab10afa1a0e2dba7853ed5f1e5bf1c631757ed4e103b593ff3f5620",
                         "45c0b7f8c09a9e1f1cea0c25785594427b6bf8f9f878a8af0b1abbb48e16d0920d8becd0c220f67c51217eecfd718"
                         "4ef0732481c843857e6bc7fc095c4f6b788",
                         0}};
    ...
    EXPECT_NE(recoveredPoints[0].value(), recoveredPoints[1].value())
        << "Different messages unexpectedly recovered to same point";
}