...
protected:
    void SetUp() override {
        ...
        for (int bit = 0; bit < 32 * 8; ++bit) { // Test each bit flip
...
TEST_F(Murmur32Test, DataAlignmentTesting) {
    ...
    std::vector<io::ByteVector> alignment_tests = {
        unaligned_1byte, unaligned_2bytes, unaligned_3bytes,
        aligned_4bytes, unaligned_5bytes, aligned_8bytes, aligned_16bytes
    };
...
TEST_F(Murmur32Test, PatternDistributionTesting) {
    ...
    std::vector<io::ByteVector> patterns = {
        pattern_zeros, pattern_0xff, pattern_0xaa, pattern_0x55, pattern_increment
    };
...
TEST_F(Murmur32Test, AvalancheEffectTesting) {
    ...
    for (const auto& variant : avalanche_variants) {
        ...
        if (bit_diff >= 8) { // At least 25% bit change
            significant_changes++;
        }
    }
...
TEST_F(Murmur32Test, SeedBoundaryTesting) {
    ...
    std::vector<uint32_t> boundary_seeds = {
        0x00000000, 0x00000001, 0x7FFFFFFF, 0x80000000, 0x80000001, 0xFFFFFFFE, 0xFFFFFFFF
    };
...
TEST_F(Murmur32Test, ThreadSafetyTesting) {
    ...
    for (int i = 0; i < 10; ++i) {
        threads.emplace_back([&results, i, this]() {
...
TEST_F(Murmur32Test, ComprehensiveRegressionTesting) {
    ...
    std::vector<TestVector> test_vectors = {
        {"", 0, 0}, // These need to be filled with actual C# results
        {"00", 0, 0},
        {"616263", 0, 0},
        {"48656c6c6f", 1, 0},
    ...
    for (const auto& tv : test_vectors) {
        if (tv.expected_hash != 0) { // Only test if we have expected value
            ...
            EXPECT_EQ(actual_hash, tv.expected_hash) 
                << "Regression test failed for input: " << tv.input_hex 
                << " seed: " << tv.seed;
        }
    }
}
