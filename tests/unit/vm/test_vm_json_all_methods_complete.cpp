...
class TestEngine : public ExecutionEngine
{
  public:
    ...
    std::string GetFaultException() const
    {
        return fault_exception_;
    }
...
class VMJsonTestBase
{
  protected:
    ...
    void ValidateExecutionContext(const VMUTExecutionContextState& expected, std::shared_ptr<ExecutionContext> actual)
    {
        EXPECT_EQ(expected.instruction_pointer, actual->InstructionPointer());

        if (actual->InstructionPointer() < actual->Script().size())
        {
            EXPECT_EQ(expected.next_instruction, static_cast<OpCode>(actual->Script()[actual->InstructionPointer()]));
        }
...
VMUTStackItem VMUTStackItem::FromJson(const nlohmann::json& j)
{
    ...
    if (j.contains("type"))
    {
        ...
        if (type_str == "Null")
            item.type = VMUTStackItemType::Null;
        else if (type_str == "Pointer")
            item.type = VMUTStackItemType::Pointer;
        else if (type_str == "Boolean")
            item.type = VMUTStackItemType::Boolean;
        else if (type_str == "ByteString")
            item.type = VMUTStackItemType::ByteString;
        else if (type_str == "String")
            item.type = VMUTStackItemType::String;
        else if (type_str == "Buffer")
            item.type = VMUTStackItemType::Buffer;
        else if (type_str == "Interop")
            item.type = VMUTStackItemType::Interop;
        else if (type_str == "Integer")
            item.type = VMUTStackItemType::Integer;
        else if (type_str == "Array")
            item.type = VMUTStackItemType::Array;
        else if (type_str == "Struct")
            item.type = VMUTStackItemType::Struct;
        else if (type_str == "Map")
            item.type = VMUTStackItemType::Map;
    }
...
nlohmann::json VMUTStackItem::ToJson() const
{
    ...
    switch (type)
    {
        ...
        case VMUTStackItemType::Buffer:
            j["type"] = "Buffer";
            break;
        case VMUTStackItemType::Interop:
            j["type"] = "Interop";
            break;
        case VMUTStackItemType::Integer:
            j["type"] = "Integer";
...
VMUTExecutionEngineState VMUTExecutionEngineState::FromJson(const nlohmann::json& j)
{
    ...
    if (j.contains("state"))
    {
        ...
        if (state_str == "NONE")
            state.state = VMState::None;
        else if (state_str == "HALT")
            state.state = VMState::Halt;
        else if (state_str == "FAULT")
            state.state = VMState::Fault;
        else if (state_str == "BREAK")
            state.state = VMState::Break;
    }
