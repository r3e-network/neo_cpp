...
class ViewChangeRecoveryTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        EXPECT_CALL(*settings_, GetStandbyCommittee()).WillRepeatedly(ReturnRef(validators_));
    }
    ...
    std::shared_ptr<ChangeViewMessage> CreateChangeViewMessage(uint8_t validator_index, uint8_t new_view_number,
    ...
    std::shared_ptr<RecoveryMessage> CreateRecoveryMessage(uint8_t validator_index, uint8_t view_number)
    {
...
TEST_F(ViewChangeRecoveryTest, ViewChangeMessageProcessing)
{
    ...
    {  // 4 validators send change view to view 1
    ...
    {  // M = (7+1)*2/3 = 5.33, so need 6, but for view change might be different
        EXPECT_EQ(context->GetViewNumber(), 1);
    }
}
...
TEST_F(ViewChangeRecoveryTest, RecoveryRequestMechanism)
{
    ...
    {
        if (i != 3)
        {  // Don't include the requesting node itself
    ...
    bool any_delivered =
        std::any_of(delivery_results.begin(), delivery_results.end(), [](bool result) { return result; });
