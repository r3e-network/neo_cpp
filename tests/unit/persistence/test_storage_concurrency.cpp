...
protected:
    ...
    enum class OperationType { Read, Write, Delete, Find, Commit };
...
TEST_F(StorageConcurrencyTest, ConcurrentReads) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, operations_per_thread, &successful_reads, &failed_reads]() {
...
TEST_F(StorageConcurrencyTest, ConcurrentWritesDifferentKeys) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, t, operations_per_thread, &successful_writes]() {
            for (int i = 0; i < operations_per_thread; ++i) {
    ...
    for (size_t i = 0; i < std::min(test_keys_.size(), 
                                   static_cast<size_t>(num_threads * operations_per_thread)); ++i) {
...
TEST_F(StorageConcurrencyTest, ConcurrentWritesSameKey) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, &target_key, t, &successful_updates, &failed_updates]() {
            try {
                StorageItem item;
...
TEST_F(StorageConcurrencyTest, ConcurrentReadWrites) {
    ...
    for (int t = 0; t < num_reader_threads; ++t) {
        threads.emplace_back([this, operations_per_thread, &reads_completed]() {
            for (int i = 0; i < operations_per_thread; ++i) {
    ...
    for (int t = 0; t < num_writer_threads; ++t) {
        threads.emplace_back([this, t, operations_per_thread, &writes_completed]() {
            for (int i = 0; i < operations_per_thread; ++i) {
...
TEST_F(StorageConcurrencyTest, ConcurrentDeletes) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, t, &successful_deletes]() {
...
TEST_F(StorageConcurrencyTest, ConcurrentCommits) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, t, &successful_commits, &failed_commits]() {
            try {
...
TEST_F(StorageConcurrencyTest, ConcurrentFind) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, finds_per_thread, &successful_finds]() {
...
TEST_F(StorageConcurrencyTest, CacheCoherencyUnderConcurrency) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, &test_key, &iteration, num_iterations, t, &observed_values]() {
            while (iteration.load() < num_iterations) {
                ...
                if (t == 0) {
                    ...
                    std::vector<uint8_t> value = {
                        static_cast<uint8_t>(current_iter & 0xFF),
...
TEST_F(StorageConcurrencyTest, DeadlockPrevention) {
    ...
    for (int t = 0; t < num_threads; ++t) {
        threads.emplace_back([this, t, operations_per_thread, &deadlock_detected, &completed_threads]() {
            try {
                for (int i = 0; i < operations_per_thread; ++i) {
...
TEST_F(StorageConcurrencyTest, MemoryConsistencyHighConcurrency) {
    ...
    for (int t = 0; t < num_producer_threads; ++t) {
        threads.emplace_back([this, t, items_per_producer, &items_produced]() {
    ...
    for (int t = 0; t < num_consumer_threads; ++t) {
        threads.emplace_back([this, &items_consumed, &production_complete, items_per_producer, num_producer_threads]() {
            ...
            while (!production_complete.load() || items_consumed.load() < expected_total) {
                ...
                if (items_consumed.load() >= expected_total) break;
            }
        });
    }
