...
class StoreCacheTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        auto uniqueSnapshot = store->GetSnapshot();
        snapshot = std::shared_ptr<IStoreSnapshot>(std::move(uniqueSnapshot));
    }
...
// Note: TestConstructorNullStore test removed because StoreCache
// constructor takes a reference, not a pointer, so nullptr cannot be passed
...
{
    // Verify the key exists in snapshot first
    StorageKey key1(1, {0x01, 0x02});
