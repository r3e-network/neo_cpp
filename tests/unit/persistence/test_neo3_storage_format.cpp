...
class Neo3StorageFormatTest : public ::testing::Test
{
  protected:
    ...
    StorageKey CreateNeo3StorageKey(int32_t contract_id, uint8_t prefix, const std::vector<uint8_t>& key_data = {})
    {
    ...
    bool VerifyNeo3Serialization(const StorageKey& key)
    {
        ...
        int32_t decoded_id = static_cast<int32_t>(serialized[0]) | (static_cast<int32_t>(serialized[1]) << 8) |
                             (static_cast<int32_t>(serialized[2]) << 16) | (static_cast<int32_t>(serialized[3]) << 24);
...
TEST_F(Neo3StorageFormatTest, PerformanceWithNeo3Format)
{
    ...
    {
        auto key = StorageKey::Create(test_contract_id_, 0x01, TestHelpers::GenerateRandomBytes(20));
        StorageItem item;
