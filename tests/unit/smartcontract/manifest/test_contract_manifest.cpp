...
TEST_F(ContractManifestTest, ParseFromJson_NEP17Token)
{
    ...
    // Test symbol method
    auto symbol_method =
        std::find_if(methods.begin(), methods.end(), [](const auto& m) { return m.GetName() == "symbol"; });
    ...
    auto transfer_method =
        std::find_if(methods.begin(), methods.end(), [](const auto& m) { return m.GetName() == "transfer"; });
...
TEST_F(ContractManifestTest, ParseFromJson_SafeMethods)
{
    ...
    auto safe_method =
        std::find_if(methods.begin(), methods.end(), [](const auto& m) { return m.GetName() == "safeMethod"; });
    auto unsafe_method =
        std::find_if(methods.begin(), methods.end(), [](const auto& m) { return m.GetName() == "unsafeMethod"; });
...
TEST_F(ContractManifestTest, TestGenerator)
{
    ...
    auto manifest = builder.SetName("GeneratedContract")
