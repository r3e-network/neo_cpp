...
class NeoTokenAllMethodsTest : public ::testing::Test
{
  protected:
    ...
    {
        auto engine = ApplicationEngine::Create(TriggerType::Application, nullptr, cache, persisting_block,
...
TEST_F(NeoTokenAllMethodsTest, Test_HF_EchidnaStates)
{
    ...
    for (const auto& method : methods)
    {
        ...
        {
            auto engine =
                ApplicationEngine::Create(TriggerType::Application, nullptr, cloned_cache, persisting_block, settings);
...
TEST_F(NeoTokenAllMethodsTest, Check_RegisterValidator)
{
    ...
    auto engine = ApplicationEngine::Create(TriggerType::Application, nullptr, cloned_cache, persisting_block,
                                            GetTestProtocolSettings());
...
{
    auto validators =
        NativeContract::NEO::GetNextBlockValidators(snapshot_cache_, GetTestProtocolSettings().ValidatorsCount);
