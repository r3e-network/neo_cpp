...
TEST_F(NeoTokenGovernanceTest, GetCommittee_AfterVoting)
{
    ...
    for (int i = 0; i < 25; i++)
    {
        ...
        ECPoint candidate = i < test_candidates_.size() ? test_candidates_[i] : KeyPair::Generate().PublicKey;
...
TEST_F(NeoTokenGovernanceTest, CompleteElectionCycle)
{
    ...
    auto candidates = neo_token_->GetCandidates(data_cache_.get());
    std::sort(candidates.begin(), candidates.end(), [](const auto& a, const auto& b) { return a.Votes > b.Votes; });
