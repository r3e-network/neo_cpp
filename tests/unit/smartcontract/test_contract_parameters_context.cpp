...
class UT_ContractParametersContext : public testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        context = std::make_unique<ContractParametersContext>(*dataCache, static_cast<const IVerifiable&>(*transaction),
                                                              0x4F454E);

        // Create a key pair for testing
        privateKey = ByteVector(std::vector<uint8_t>(32, 0x01));
        keyPair =
            ECPoint::FromHex("0279BE667EF9DCBBAC55A06295CE870B07029BFCDB2DCE28D959F2815B16F81798");  // dummy public key
...
TEST_F(UT_ContractParametersContext, TestAddMultipleParameters)
{
    ...
    multiParamContract.SetParameterList(
        {ContractParameterType::Signature, ContractParameterType::Boolean, ContractParameterType::Integer});

    // Add multiple parameters to the contract
    std::vector<ByteVector> parameters = {ByteVector(std::vector<uint8_t>(64, 0x01)), ByteVector::FromHexString("01"),
