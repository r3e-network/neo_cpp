...
namespace neo::console_service::tests
{
    ...
    TEST_F(ConsoleServiceBaseTest, TestCommandHandlerRegistration)
    {
        ...
        EXPECT_NO_THROW({
            service->RegisterCommandHandler<std::string>([](std::vector<std::shared_ptr<CommandToken>>& args, bool consume_all) -> std::string {
                return "test";
            });
        });
        ...
        EXPECT_NO_THROW({
            service->RegisterCommandHandler<int>([](std::vector<std::shared_ptr<CommandToken>>& args, bool consume_all) -> int {
                return 42;
            });
        });
    }
