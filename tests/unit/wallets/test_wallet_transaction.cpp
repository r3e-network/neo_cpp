...
TEST_F(UT_WalletTransaction, TestJsonSerialization)
{
    ...
    auto originalMillis =
        std::chrono::duration_cast<std::chrono::milliseconds>(originalTime.time_since_epoch()).count();
    auto deserializedMillis =
        std::chrono::duration_cast<std::chrono::milliseconds>(deserializedTx.GetTime().time_since_epoch()).count();
