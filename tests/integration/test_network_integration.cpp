...
TEST_F(NetworkIntegrationTest, MultipleNodeNetwork)
{
    ...
    for (int i = 0; i < num_nodes; ++i)
    {
        EXPECT_GT(servers[i]->GetConnectedPeersCount(), 0) << "Node " << i << " has no connections";
    }
...
TEST_F(NetworkIntegrationTest, NetworkStressTest)
{
    ...
    for (int t = 0; t < num_threads; ++t)
    {
        threads.emplace_back(
            [this, messages_per_thread, &messages_sent, &errors]()
            {
...
TEST_F(NetworkIntegrationTest, ConcurrentConnections)
{
    ...
    for (int i = 0; i < num_concurrent_clients; ++i)
    {
        ...
        auto future =
            std::async(std::launch::async, [server, this]() { return server->ConnectToPeer("127.0.0.1", base_port_); });
