...
public:
    ...
    uint16_t GetRPCPort() const { return rpc_port_; }
    std::shared_ptr<node::NeoSystem> GetNeoSystem() const { return neo_system_; }
...
protected:
    ...
    void AssertConsensusProgress(std::shared_ptr<TestNetwork> network, 
                               uint32_t expected_blocks, 
                               std::chrono::seconds timeout = std::chrono::seconds(60));
    
    // Scenario generators
    std::vector<std::shared_ptr<ledger::Transaction>> GenerateTestTransactions(int count);
    std::shared_ptr<ledger::Block> GenerateTestBlock(uint32_t index, 
    ...
    PerformanceMetrics MeasurePerformance(std::shared_ptr<TestNetwork> network, 
...
protected:
    ...
    std::string SendRPCRequest(std::shared_ptr<TestNode> node, 
