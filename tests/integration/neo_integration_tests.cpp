...
class NeoIntegrationTestBase : public ::testing::Test
{
  ...
  protected:
     ...
     */
    std::shared_ptr<ledger::Transaction> CreateTestTransaction(int64_t systemFee = 1000000,
...
TEST_F(BlockchainOperationsTest, GenesisBlockValidation)
{
    ...
    auto expectedGenesisHash =
        io::UInt256::Parse("0x1f4d1defa46faa5e7b9b8d3f79a06bec777d7c26c4aa5dbaec5a06bec777d7c26");
...
TEST_F(NetworkProtocolTest, MessageSerialization)
{
    ...
    auto versionMessage = network::VersionMessage::Create(protocolSettings_->GetNetwork(),
                                                          12345,  // Port
                                                          67890,  // Nonce
                                                          "Neo C++ Node");
...
TEST_F(NetworkProtocolTest, InventoryMessageHandling)
{
    ...
    auto invMessage = network::InventoryMessage::Create(network::InventoryType::Transaction, hashes);
...
TEST_F(PerformanceBenchmarkTest, TransactionValidationPerformance)
{
    ...
    std::cout << "Transaction validation performance: " << avgTimePerTx << " μs per transaction" << std::endl;
}
...
TEST_F(PerformanceBenchmarkTest, BlockProcessingPerformance)
{
    ...
    std::cout << "Block processing performance: " << avgTimePerBlock << " ms per block" << std::endl;
}
