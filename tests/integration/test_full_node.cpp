...
namespace neo::tests::integration
{
    ...
    protected:
        ...
        void SetUp() override
        {
            ...
            health_->Register("database", 
                std::make_shared<monitoring::DatabaseHealthCheck>(system_->GetBlockchain()));
            health_->Register("network",
                std::make_shared<monitoring::NetworkHealthCheck>(node_));
        }
    ...
    TEST_F(FullNodeIntegrationTest, BlockSynchronization)
    {
        ...
        auto db2 = std::make_shared<persistence::RocksDbStore>(
            persistence::RocksDbConfig{.db_path = "./test_data/rocksdb2"});
        db2->Open();
        ...
        EXPECT_EQ(system2->GetBlockchain()->GetHeight(), 
                  system_->GetBlockchain()->GetHeight());
    ...
    TEST_F(FullNodeIntegrationTest, ConsensusOperation)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            auto consensus = std::make_shared<consensus::DbftConsensus>(
                config, validators[i], validators);
    ...
    {
        auto block_height = metrics_->Register<monitoring::Gauge>(
            "neo_block_height", "Current blockchain height");
        auto peer_count = metrics_->Register<monitoring::Gauge>(
            "neo_peer_count", "Number of connected peers");
        auto tx_count = metrics_->Register<monitoring::Counter>(
            "neo_transactions_total", "Total transactions processed");
