...
TEST_F(PerformanceTest, MemoryPoolPerformance)
{
    ...
    std::cout << "Memory pool sort time for " << num_transactions << " transactions: " << duration.count() << "ms"
              << std::endl;
...
TEST_F(PerformanceTest, ConcurrentAccessPerformance)
{
    ...
    std::cout << "Concurrent operations per second (" << num_threads << " threads): " << ops_per_second << std::endl;
