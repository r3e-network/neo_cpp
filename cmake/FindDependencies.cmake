...
if(Boost_FOUND)
    ...
    # Ensure imported targets exist even when the module finds only library files
    if(NOT TARGET Boost::boost AND Boost_INCLUDE_DIRS)
        add_library(Boost::boost INTERFACE IMPORTED)
        target_include_directories(Boost::boost INTERFACE ${Boost_INCLUDE_DIRS})
    endif()

    set(BOOST_LIBRARIES "")
    foreach(_boost_comp IN ITEMS system filesystem thread)
        string(TOUPPER ${_boost_comp} _BOOST_COMP_UPPER)
        set(_boost_lib "")
        if(DEFINED Boost_${_BOOST_COMP_UPPER}_LIBRARY AND Boost_${_BOOST_COMP_UPPER}_LIBRARY)
            set(_boost_lib "${Boost_${_BOOST_COMP_UPPER}_LIBRARY}")
        elseif(DEFINED Boost_${_BOOST_COMP_UPPER}_LIBRARY_RELEASE AND Boost_${_BOOST_COMP_UPPER}_LIBRARY_RELEASE)
            set(_boost_lib "${Boost_${_BOOST_COMP_UPPER}_LIBRARY_RELEASE}")
        elseif(DEFINED Boost_${_BOOST_COMP_UPPER}_LIBRARY_DEBUG AND Boost_${_BOOST_COMP_UPPER}_LIBRARY_DEBUG)
            set(_boost_lib "${Boost_${_BOOST_COMP_UPPER}_LIBRARY_DEBUG}")
        endif()

        if(NOT TARGET Boost::${_boost_comp} AND _boost_lib)
            add_library(Boost::${_boost_comp} UNKNOWN IMPORTED)
            set_target_properties(Boost::${_boost_comp} PROPERTIES IMPORTED_LOCATION "${_boost_lib}")
            target_link_libraries(Boost::${_boost_comp} INTERFACE Boost::boost)
        endif()

        if(TARGET Boost::${_boost_comp})
            list(APPEND BOOST_LIBRARIES Boost::${_boost_comp})
        endif()
    endforeach()

    if(BOOST_LIBRARIES STREQUAL "" AND Boost_LIBRARIES)
        # Fallback to the classic library list if imported targets could not be created
