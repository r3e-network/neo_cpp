    ...
    : config_(config), logger_(core::LoggerFactory::GetLogger("RocksDbStore"))
{
    ...
    VALIDATE_RANGE(config_.write_buffer_size, static_cast<size_t>(1024 * 1024), static_cast<size_t>(1024ULL * 1024 * 1024));
    VALIDATE_RANGE(config_.max_write_buffer_number, 1, 16);
    VALIDATE_RANGE(config_.block_cache_size, static_cast<size_t>(1024 * 1024), static_cast<size_t>(16ULL * 1024 * 1024 * 1024));
...
bool RocksDbStore::Open()
{
    ...
    try
    {
        ...
        if (!status.ok())
        {
            throw neo::core::StorageException(
                neo::core::NeoException::ErrorCode::STORAGE_ERROR,
...
RocksDbStore::Find(const io::ByteVector* prefix, SeekDirection direction) const
{
    using namespace neo::core;
    ...
    auto operation = [this, prefix, direction]() -> std::vector<std::pair<io::ByteVector, io::ByteVector>> {
        ...
        while (it->Valid())
        {
            if (prefix != nullptr)
            {
                ...
                if (direction == SeekDirection::Forward)
                {
                    if (key_str.size() < prefix_str.size() || 
                        key_str.substr(0, prefix_str.size()) != prefix_str)
                        break;
                }
                else
                {
                    if (key_str.find(prefix_str) != 0)
                        break;
                }
            }
...
std::string RocksDbStore::GetStatistics() const
{
    ...
    if (!IsOpen() || !options_ || !options_->statistics)
        return "Database not open or statistics not enabled";
...
std::string RocksDbStore::GetProperty(const std::string& property) const
{
    ...
    std::string value;
    if (db_->GetProperty(default_cf_, property, &value))
        return value;
...
{
    if (key.Size() == 0 || cf_handles_.empty())
        return default_cf_;
...
void RocksDbStore::WriteBatch::Clear()
{
    batch_.Clear();
}

size_t RocksDbStore::WriteBatch::GetDataSize() const
{
...
std::vector<std::pair<io::ByteVector, io::ByteVector>> 
RocksDbStore::Find(const io::ByteVector* prefix, SeekDirection direction) const { return {}; }
