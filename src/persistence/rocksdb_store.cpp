...
RocksDBStore::RocksDBStore(const std::string& path) : path_(path)
{
...
std::optional<io::ByteVector> RocksDBStore::TryGet(const io::ByteVector& key) const
{
    ...
    rocksdb::Status status =
        db_->Get(rocksdb::ReadOptions(), rocksdb::Slice(reinterpret_cast<const char*>(key.Data()), key.Size()), &value);
                                                                          ...
                                                                          SeekDirection direction) const
{
    ...
    while (it->Valid())
    {
        ...
        io::ByteVector valueVector(
            io::ByteSpan(reinterpret_cast<const uint8_t*>(it->value().data()), it->value().size()));
...
void RocksDBStore::Put(const io::ByteVector& key, const io::ByteVector& value)
{
    ...
    rocksdb::Status status = db_->Put(options, rocksdb::Slice(reinterpret_cast<const char*>(key.Data()), key.Size()),
...
void RocksDBStore::Delete(const io::ByteVector& key)
{
    rocksdb::WriteOptions options;
    rocksdb::Status status =
        db_->Delete(options, rocksdb::Slice(reinterpret_cast<const char*>(key.Data()), key.Size()));
...
std::optional<io::ByteVector> RocksDBSnapshot::TryGet(const io::ByteVector& key) const
{
    ...
    std::string value;
    rocksdb::Status status =
        store_.db_->Get(options, rocksdb::Slice(reinterpret_cast<const char*>(key.Data()), key.Size()), &value);
...
std::vector<std::pair<io::ByteVector, io::ByteVector>> RocksDBSnapshot::Find(const io::ByteVector* prefix,
                                                                             SeekDirection direction) const
{
    std::vector<std::pair<io::ByteVector, io::ByteVector>> result;
...
{
    batch_->Put(rocksdb::Slice(reinterpret_cast<const char*>(key.Data()), key.Size()),
                rocksdb::Slice(reinterpret_cast<const char*>(value.Data()), value.Size()));
}
