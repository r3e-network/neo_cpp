...
namespace neo::plugins
{
    StatisticsPlugin::StatisticsPlugin()
        : PluginBase("Statistics", "Collects and reports node statistics", "1.0", "Neo C++ Team"),
          blockCount_(0), transactionCount_(0), peerCount_(0), memoryPoolSize_(0),
          interval_(60), blockCallbackId_(-1), transactionCallbackId_(-1)
    {
    }
    ...
    bool StatisticsPlugin::OnStart()
    {
        ...
        // Register callbacks
        blockCallbackId_ = node->RegisterBlockPersistenceCallback(std::bind(&StatisticsPlugin::OnBlockPersisted, this, std::placeholders::_1));
        transactionCallbackId_ = node->RegisterTransactionExecutionCallback(std::bind(&StatisticsPlugin::OnTransactionExecuted, this, std::placeholders::_1));
