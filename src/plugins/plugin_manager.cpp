...
namespace neo::plugins
{
    ...
    bool PluginManager::LoadPlugins(std::shared_ptr<node::NeoSystem> neoSystem, const std::unordered_map<std::string, std::string>& settings)
    {
        bool result = true;
        ...
        for (const auto& factory : factories_)
        {
            try
            {
                ...
                if (plugin->Initialize(neoSystem, settings))
                {
                    ...
                    std::cout << "Loaded plugin: " << plugin->GetName() << " v" << plugin->GetVersion() << " by " << plugin->GetAuthor() << std::endl;
                }
                else
                {
