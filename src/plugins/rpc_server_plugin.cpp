...
namespace neo::plugins
{
    RpcServerPlugin::RpcServerPlugin()
        : PluginBase("RpcServer", "Provides RPC server functionality", "1.0", "Neo C++ Team"),
          port_(10332), enableCors_(false), enableAuth_(false)
    {
    }
    ...
    bool RpcServerPlugin::OnInitialize(const std::unordered_map<std::string, std::string>& settings)
    {
        ...
        // Create RPC server
        rpcServer_ = std::make_shared<rpc::RPCServer>(GetNeoSystem(), port_, enableCors_, enableAuth_, username_, password_);

        // Register methods
        RegisterMethod("ping", [](const nlohmann::json& params) {
            return "pong";
        });

        RegisterMethod("echo", [](const nlohmann::json& params) {
            if (params.empty())
        ...
        RegisterMethod("time", [](const nlohmann::json& params) {
    ...
    void RpcServerPlugin::RegisterMethod(const std::string& method, std::function<nlohmann::json(const nlohmann::json&)> handler)
    {
        if (!rpcServer_)
            return;
