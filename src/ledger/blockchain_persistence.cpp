...
namespace neo::ledger
{
    ...
    void Blockchain::ProcessUnverifiedBlocks(uint32_t height)
    {
        ...
        for (const auto& unverified_block : unverified_list->blocks) {
            ...
            processing_queue_.push([this, unverified_block]() {
                OnNewBlock(unverified_block);
            });
            processing_cv_.notify_one();
        }
    }

    std::vector<ApplicationExecuted> Blockchain::ExecuteBlockScripts(
    ...
    void Blockchain::FireCommittingEvent(std::shared_ptr<Block> block, 
                                       std::shared_ptr<persistence::DataCache> snapshot,
