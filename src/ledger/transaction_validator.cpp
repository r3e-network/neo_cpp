...
class TransactionValidator
{
   public:
    ...
    static ValidationResult ValidateTransaction(const Transaction& tx, std::shared_ptr<Blockchain> blockchain,
                                                std::shared_ptr<MemoryPool> mempool = nullptr)
    {
        ...
        {
            LOG_WARNING("Transaction expired at block {}, current height {}", tx.GetValidUntilBlock(), currentHeight);
   ...
   private:
    ...
    static ValidationResult ValidateFees(const Transaction& tx, std::shared_ptr<Blockchain> blockchain)
    {
        ...
        auto gasContract =
            smartcontract::native::NativeContractManager::GetInstance()->GetContract<smartcontract::native::GasToken>();
    ...
    static ValidationResult ValidateWitnesses(const Transaction& tx, std::shared_ptr<Blockchain> blockchain)
    {
        ...
        {
            LOG_WARNING("Signer count {} doesn't match witness count {}", signers.size(), witnesses.size());
            return ValidationResult::InvalidWitness;
        }
                                     ...
                                     std::shared_ptr<Blockchain> blockchain)
    {
        ...
        if (tx.GetSigners().empty()) return false;
                                                ...
                                                std::shared_ptr<Blockchain> blockchain)
    {
        ...
        if (attr->GetType() != TransactionAttributeType::NotValidBefore) return true;
                                           ...
                                           std::shared_ptr<Blockchain> blockchain)
    {
        ...
        if (attr->GetType() != TransactionAttributeType::Conflicts) return true;
    ...
    static bool ContainsForbiddenOpcodes(const vm::Script& script)
    {
        ...
        for (const auto& opcode : opcodes)
        {
            ...
            if (opcode == vm::OpCode::SYSCALL)
            {
                ...
                if (IsForbiddenSyscall(syscallName)) return true;
            }
        }
    ...
    static bool VerifyScriptLimits(const vm::Script& script)
    {
        ...
        if (script.GetLength() > MAX_SCRIPT_SIZE) return false;

        // Estimate maximum stack usage
        size_t estimatedStackUsage = script.EstimateStackUsage();
        if (estimatedStackUsage > MAX_STACK_SIZE) return false;

        return true;
    }

    static bool VerifyWitness(const Transaction& tx, std::shared_ptr<Signer> signer, std::shared_ptr<Witness> witness,
                              std::shared_ptr<Blockchain> blockchain)
    {
        ...
        try
        {
            ...
            if (verificationScript.size() < 35) return false;
            ...
            // Extract signature
            if (invocationScript.size() < 65) return false;
