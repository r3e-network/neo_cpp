...
{
MemoryPool::MemoryPool(size_t max_capacity) : max_capacity_(max_capacity) {}
...
bool MemoryPool::TryAdd(const network::p2p::payloads::Neo3Transaction& transaction)
{
    ...
    if (transactions_.size() >= max_capacity_)
    {
        ...
        if (transactions_.size() >= max_capacity_)
        {
            ...
            auto min_it =
                std::min_element(transactions_.begin(), transactions_.end(), [this](const auto& a, const auto& b)
                                 { return CalculatePriority(a.second) < CalculatePriority(b.second); });

            if (min_it != transactions_.end() && new_priority <= CalculatePriority(min_it->second))
            {
...
void MemoryPool::EvictLowestPriority()
{
    ...
    auto min_it = std::min_element(transactions_.begin(), transactions_.end(), [this](const auto& a, const auto& b)
                                   { return CalculatePriority(a.second) < CalculatePriority(b.second); });
