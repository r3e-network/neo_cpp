...
BlockStorage::BlockStorage(std::shared_ptr<persistence::DataCache> dataCache) : dataCache_(dataCache) {}
...
bool BlockStorage::AddBlock(const Block& block, std::shared_ptr<persistence::DataCache> snapshot)
{
    ...
    try
    {
        ...
        persistence::StorageItem blockItem(
        ...
        persistence::StorageKey headerKey(io::UInt160(),
                                          io::ByteVector::Concat(headerPrefixVector.AsSpan(), hash.AsSpan()));
        persistence::StorageItem headerItem(
            io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(headerData.data()), headerData.size())));
        ...
        persistence::StorageKey blockHashKey(
            io::UInt160(), io::ByteVector::Concat(hashPrefixVector.AsSpan(), hashIndexVector.AsSpan()));
        persistence::StorageItem blockHashItem(
            io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(blockHashData.data()), blockHashData.size())));
        ...
        if (block.GetIndex() > 0)
        {
            ...
            persistence::StorageKey nextBlockHashKey(
                io::UInt160(), io::ByteVector::Concat(nextPrefixVector.AsSpan(), block.GetPrevHash().AsSpan()));
            persistence::StorageItem nextBlockHashItem(io::ByteVector(
                io::ByteSpan(reinterpret_cast<const uint8_t*>(nextBlockHashData.data()), nextBlockHashData.size())));
...
bool BlockStorage::AddBlockHeader(const BlockHeader& header, std::shared_ptr<persistence::DataCache> snapshot)
{
    ...
    try
    {
        ...
        if (header.GetIndex() > 0)
        {
            ...
            io::ByteVector nextPrefixVector{NextBlockHashPrefix};
            persistence::StorageKey nextBlockHashKey(
                io::UInt160(), io::ByteVector::Concat(nextPrefixVector.AsSpan(), header.GetPrevHash().AsSpan()));
            persistence::StorageItem nextBlockHashItem(io::ByteVector(
                io::ByteSpan(reinterpret_cast<const uint8_t*>(nextBlockHashData.data()), nextBlockHashData.size())));
            snapshot->Add(nextBlockHashKey, nextBlockHashItem);
        }
