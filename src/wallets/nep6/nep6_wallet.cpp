...
namespace neo::wallets::nep6
{
    ...
    NEP6Account::NEP6Account(const io::UInt160& scriptHash)
        : WalletAccount(scriptHash), deployed_(false)
    {
    }

    NEP6Account::NEP6Account(const cryptography::ecc::KeyPair& keyPair, const std::string& password, const ScryptParameters& scrypt)
    ...
    bool NEP6Account::DecryptPrivateKey(const std::string& password, const ScryptParameters& scrypt)
    {
        ...
        try
        {
            auto privateKey = cryptography::ecc::Secp256r1::FromNEP2(nep2Key_, password, scrypt.GetN(), scrypt.GetR(), scrypt.GetP());
    ...
    void NEP6Account::FromJson(const nlohmann::json& json)
    {
        ...
        if (json.contains("contract") && !json["contract"].is_null())
        {
            ...
            if (contractJson.contains("parameters") && contractJson["parameters"].is_array())
            {
                ...
                {
                    parameterList.push_back(static_cast<smartcontract::ContractParameterType>(parameterJson["type"].get<uint8_t>()));
    ...
    bool NEP6Wallet::ChangePassword(const std::string& oldPassword, const std::string& newPassword)
    {
        ...
        for (const auto& account : GetAccounts())
        {
            ...
            if (nep6Account && !nep6Account->GetNEP2Key().empty())
            {
                ...
                auto nep2Key = cryptography::ecc::Secp256r1::ToNEP2(privateKey, newPassword, scrypt_.GetN(), scrypt_.GetR(), scrypt_.GetP());
    ...
    {
        // Decrypt private key
        auto privateKey = cryptography::ecc::Secp256r1::FromNEP2(nep2Key, password, scrypt_.GetN(), scrypt_.GetR(), scrypt_.GetP());
