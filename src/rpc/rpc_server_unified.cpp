...
{
public:
    RpcServerImpl(std::shared_ptr<node::NeoSystem> neoSystem, uint16_t port, 
                  bool enableCors, bool enableAuth,
...
private:
    ...
    {
        // Main RPC endpoint
        httpServer_->Post("/", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        httpServer_->Get("/health", [this](const httplib::Request& req, httplib::Response& res) {
            nlohmann::json response = {
                {"status", "healthy"},
                {"version", "1.0.0"},
                {"network", "mainnet"} // TODO: Get network name from NeoSystem when available
            };
            res.set_content(response.dump(), "application/json");
        });
        
        // Metrics endpoint
        httpServer_->Get("/metrics", [this](const httplib::Request& req, httplib::Response& res) {
        ...
        if (it != methods_.end())
        {
            ...
            catch (const std::exception& e)
            {
                return {
                    ...
                    {"error", {
                        {"code", -32603},
                        {"message", "Method execution failed: " + std::string(e.what())}
                    }},
                    {"id", request.contains("id") ? request["id"] : nullptr}
                };
            }
    ...
    nlohmann::json GetMetrics()
    {
        ...
        return {
            {"blockchain", {
                ...
                {"header_height", blockchain ? blockchain->GetHeight() : 0} // TODO: Implement separate header height tracking
            }},
            {"mempool", {
                {"size", mempool ? mempool->GetSize() : 0},
                {"verified", mempool ? mempool->GetSize() : 0} // TODO: Track verified transactions separately
            }},
            {"rpc", {
...
private:
    ...
    std::unordered_map<std::string, std::function<nlohmann::json(std::shared_ptr<node::NeoSystem>, const nlohmann::json&)>> methods_;
