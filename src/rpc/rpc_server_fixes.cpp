...
//
// rpc_server_fixes.cpp file belongs to the neo project and is free
                ...
                if (neoContract) {
                    ...
                    if (balanceResult) {
                        ...
                        if (balance > 0) {
                            ...
                            neoBalance.SetProperty("assethash", std::make_shared<json::JString>(neoContract->GetScriptHash().ToString()));
                            neoBalance.SetProperty("amount", std::make_shared<json::JString>(std::to_string(balance)));
                            neoBalance.SetProperty("lastupdatedblock", std::make_shared<json::JNumber>(blockchain_->GetHeight()));
                ...
                if (gasContract) {
                    ...
                    if (balanceResult) {
                        ...
                        if (balance > 0) {
                            ...
                            gasBalance.SetProperty("assethash", std::make_shared<json::JString>(gasContract->GetScriptHash().ToString()));
                            gasBalance.SetProperty("amount", std::make_shared<json::JString>(std::to_string(balance)));
                            gasBalance.SetProperty("lastupdatedblock", std::make_shared<json::JNumber>(blockchain_->GetHeight()));
            ...
            // Add other NEP-17 token balances by iterating through contracts
            {
                ...
                if (contractManagement) {
                    ...
                    for (const auto& contract : contracts) {
                        ...
                        bool hasBalanceOf = contractManagement->HasMethod(snapshot, contract->GetScriptHash(), "balanceOf", 1);
                        bool hasSymbol = contractManagement->HasMethod(snapshot, contract->GetScriptHash(), "symbol", 0);
                        bool hasDecimals = contractManagement->HasMethod(snapshot, contract->GetScriptHash(), "decimals", 0);
                        bool hasTotalSupply = contractManagement->HasMethod(snapshot, contract->GetScriptHash(), "totalSupply", 0);
                        bool hasTransfer = contractManagement->HasMethod(snapshot, contract->GetScriptHash(), "transfer", 4);
                        ...
                        if (hasBalanceOf && hasSymbol && hasDecimals && hasTotalSupply && hasTransfer) {
                            try {
                                ...
                                engine.CallContract(contract->GetScriptHash(), "balanceOf", args, smartcontract::CallFlags::ReadStates);
                                ...
                                if (engine.Execute() == vm::VMState::HALT && engine.GetResultStack().GetCount() > 0) {
                                    ...
                                    if (balanceItem && balanceItem->IsInteger()) {
                                        ...
                                        if (balance > 0) {
                                            ...
                                            tokenBalance.SetProperty("assethash", std::make_shared<json::JString>(contract->GetScriptHash().ToString()));
                                            tokenBalance.SetProperty("amount", std::make_shared<json::JString>(std::to_string(balance)));
                                            tokenBalance.SetProperty("lastupdatedblock", std::make_shared<json::JNumber>(blockchain_->GetHeight()));
