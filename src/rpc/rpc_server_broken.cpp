...
RpcServer::RpcServer(const RpcConfig& config)
    : config_(config), logger_(core::Logger::GetInstance()), start_time_(std::chrono::steady_clock::now())
{
    InitializeHandlers();
}
...
void RpcServer::InitializeHandlers()
{
    ...
    method_handlers_["gettransactionheight"] = [this](const json::JArray& params)
...
smartcontract::native::NativeContract* RpcServer::GetNativeContractByID(int32_t id) const
{
    ...
    {
        case -1:
            return smartcontract::native::NeoToken::GetInstance().get();
        case -2:
            return smartcontract::native::GasToken::GetInstance().get();
        case -3:
            return smartcontract::native::PolicyContract::GetInstance().get();
        case -4:
            return smartcontract::native::ContractManagement::GetInstance().get();
        case -5:
            return smartcontract::native::LedgerContract::GetInstance().get();
        case -6:
            return smartcontract::native::StdLib::GetInstance().get();
        case -7:
            return smartcontract::native::CryptoLib::GetInstance().get();
        default:
            return nullptr;
    }
}
...
std::optional<io::ByteVector> RpcServer::GetStorageValue(const io::UInt160& contract_hash,
...
json::JObject RpcServer::ExecuteContractFunction(const io::UInt160& contract_hash, const std::string& method,
                                                 const json::JArray& params) const
{
    json::JObject result;
