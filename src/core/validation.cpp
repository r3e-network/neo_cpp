...
Validator::ValidationResult Validator::ValidateSize(size_t actual_size, size_t expected_size, const std::string& name)
{
    if (actual_size != expected_size)
    {
        return ValidationResult(false,
                                name + " size (" + std::to_string(actual_size) + ") does not match expected size (" +
...
Validator::ValidationResult Validator::ValidateMinSize(size_t actual_size, size_t min_size, const std::string& name)
{
    ...
    {
        return ValidationResult(false,
                                name + " size (" + std::to_string(actual_size) + ") is less than minimum required (" +
...
Validator::ValidationResult Validator::ValidateMaxSize(size_t actual_size, size_t max_size, const std::string& name)
{
    ...
    {
        return ValidationResult(false,
                                name + " size (" + std::to_string(actual_size) + ") exceeds maximum allowed (" +
...
Validator::ValidationResult Validator::ValidateSizeRange(size_t actual_size, size_t min_size, size_t max_size,
...
Validator::ValidationResult Validator::ValidateHexString(const std::string& hex, size_t expected_length,
                                                         const std::string& name)
{
...
Validator::ValidationResult Validator::ValidateByteSpan(const io::ByteSpan& span, size_t expected_size,
                                                        const std::string& name)
{
...
Validator::ValidationResult Validator::ValidateNoSQLInjection(const std::string& input, const std::string& name)
{
    ...
    std::vector<std::string> dangerous_patterns = {"'",       "--",      "/*",       "*/",      "xp_",      "sp_",
                                                   "select ", "insert ", "update ",  "delete ", "drop ",    "create ",
                                                   "alter ",  "exec ",   "execute ", "union ",  "order by", "group by"};
...
Validator::ValidationResult Validator::ValidateFileName(const std::string& filename, const std::string& name)
{
    ...
    std::vector<std::string> reserved_names = {"CON",  "PRN",  "AUX",  "NUL",  "COM1", "COM2", "COM3", "COM4",
...
Validator::ValidationResult Validator::ValidateIPAddress(const std::string& ip, const std::string& name)
{
    ...
    {
        return ValidationResult(false, name + " is not a valid IPv4 address", NeoException::ErrorCode::INVALID_FORMAT);
    }
...
Validator::ValidationResult Validator::ValidateTransactionFee(int64_t fee, int64_t max_fee)
{
    if (fee < 0)
    {
        return ValidationResult(false, "transaction fee cannot be negative", NeoException::ErrorCode::INVALID_ARGUMENT);
    }
...
Validator::ValidationResult Validator::ValidateGasAmount(int64_t gas, int64_t max_gas)
{
    if (gas < 0)
    {
        return ValidationResult(false, "gas amount cannot be negative", NeoException::ErrorCode::INVALID_ARGUMENT);
    }

    if (gas > max_gas)
    {
        return ValidationResult(false, "gas amount exceeds maximum allowed", NeoException::ErrorCode::OUT_OF_GAS);
    }
...
Validator::ValidationResult Validator::ValidateTimestamp(uint64_t timestamp)
{
    ...
    {
        return ValidationResult(false, "timestamp is too far in the future", NeoException::ErrorCode::INVALID_ARGUMENT);
    }

    if (timestamp < min_past)
    {
        return ValidationResult(false, "timestamp is too far in the past", NeoException::ErrorCode::INVALID_ARGUMENT);
    }
