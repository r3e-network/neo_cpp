...
namespace detail
{
    ...
    struct EdPoint
    {
        ...
        EdPoint(const FieldElement& x_, const FieldElement& y_) 
            : x(x_), y(y_), z(1), t(x_ * y_) {}
...
io::ByteVector Ed25519::PrivateKey::GetBytes() const
{
    return io::ByteVector(key_data_.data(), key_data_.size());
}
...
io::ByteVector Ed25519::PublicKey::GetBytes() const
{
    return io::ByteVector(key_data_.data(), key_data_.size());
}
...
std::string Ed25519::PublicKey::ToHex() const
{
    return io::ByteSpan(key_data_.data(), key_data_.size()).ToHexString();
}
...
bool Ed25519::Verify(const io::ByteSpan& message, 
