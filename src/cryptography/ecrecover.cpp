...
std::optional<ecc::ECPoint> ECRecover(const io::ByteSpan& hash, const io::ByteSpan& signature, uint8_t recovery_id)
{
    ...
    try
    {
        ...
        if (!EC_POINT_set_compressed_coordinates_GFp(group, recovered_point, x, recovery_id & 1, nullptr))
        {
            goto cleanup;
        }
        ...
        size_t point_len = EC_POINT_point2oct(group, result, POINT_CONVERSION_COMPRESSED, nullptr, 0, nullptr);
        if (point_len == 0)
        {
            goto recovery_cleanup;
        }
        ...
        if (EC_POINT_point2oct(group, result, POINT_CONVERSION_COMPRESSED, point_data.data(), point_len, nullptr) !=
            point_len)
        {
            goto recovery_cleanup;
        }
 ...
 */
std::vector<ecc::ECPoint> ECRecoverAll(const io::ByteSpan& hash, const io::ByteSpan& signature)
{
