...
namespace neo::cryptography::ecc
{
    ...
    io::ByteVector Secp256r1::ComputePublicKey(const io::ByteVector& privateKey)
    {
        ...
        size_t pubkey_len = EC_POINT_point2oct(get_ec_group(), pub_point, 
                                               POINT_CONVERSION_COMPRESSED,
                                               publicKey.data(), PUBLIC_KEY_SIZE, nullptr);
    ...
    io::ByteVector Secp256r1::WIFToPrivateKey(const std::string& wif)
    {
        ...
        if (decoded.size() < 37) { // Minimum size for WIF
    ...
    std::string Secp256r1::EncryptPrivateKey(const io::ByteVector& privateKey, const std::string& passphrase, int N, int r, int p)
    {
        if (!IsValidPrivateKey(privateKey))
            throw std::invalid_argument("Invalid private key");
        ...
        AES256::Encrypt(
            privateKey.AsSpan(),
            io::ByteSpan(derived.data(), 32), // First 32 bytes as key
    ...
    io::ByteVector Secp256r1::DecryptPrivateKey(const std::string& encryptedKey, const std::string& passphrase, int N, int r, int p)
    {
        // Base58 decode
        auto decoded = Base58::Decode(encryptedKey);
        if (decoded.size() != 43) { // NEP-2 is always 43 bytes
        ...
        if (crypto_scrypt(
            reinterpret_cast<const uint8_t*>(passphrase.data()), passphrase.size(),
            addressHash.data(), addressHash.size(),
            N, r, p,
            derived.data(), derived.size()) != 0) {
            throw std::runtime_error("Scrypt key derivation failed");
        }
