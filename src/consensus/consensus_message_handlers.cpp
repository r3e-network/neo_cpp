...
namespace neo::consensus
{
    ...
    void ConsensusService::OnChangeViewMessage(std::shared_ptr<ChangeViewMessage> message, const cryptography::ecc::ECPoint& sender)
    {
        ...
        if (message->GetViewNumber() == viewNumber_ && changeViewMessages_.find(message->GetValidatorIndex()) != changeViewMessages_.end())
            return;
    ...
    void ConsensusService::OnPrepareRequestMessage(std::shared_ptr<PrepareRequest> message, const cryptography::ecc::ECPoint& sender)
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    void ConsensusService::OnPrepareResponseMessage(std::shared_ptr<PrepareResponse> message, const cryptography::ecc::ECPoint& sender)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        // Check if message is already received
        if (message->GetViewNumber() == viewNumber_ && prepareResponses_.find(message->GetValidatorIndex()) != prepareResponses_.end())
            return;
    ...
    void ConsensusService::OnRecoveryMessage(std::shared_ptr<RecoveryMessage> message, const cryptography::ecc::ECPoint& sender)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        ...
        {
            OnPrepareRequestMessage(message->GetPrepareRequest(), validators_[message->GetPrepareRequest()->GetValidatorIndex()]);
        }
    ...
    void ConsensusService::OnRecoveryRequestMessage(std::shared_ptr<RecoveryRequest> message, const cryptography::ecc::ECPoint& sender)
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    bool ConsensusService::HasReceivedEnoughChangeViewMessages(uint8_t viewNumber) const
    {
        ...
        const auto& messages = viewNumber == this->viewNumber_ ? changeViewMessages_ :
