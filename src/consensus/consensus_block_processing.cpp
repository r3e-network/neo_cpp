...
std::shared_ptr<ledger::Block> ConsensusService::CreateBlock()
{
    ...
    std::sort(validTransactions.begin(), validTransactions.end(),
...
void ConsensusService::InitializeValidators()
{
    ...
    auto validators = roleManagement->GetDesignatedByRole(neoSystem_->GetSnapshot(),
    ...
    {
        context_ =
            std::make_shared<ConsensusContext>(validatorList, validatorIndex, keyPair_, context_->GetBlockIndex());
    }
}
