...
namespace neo::consensus
{
    ...
    std::shared_ptr<network::p2p::payloads::ExtensiblePayload> ConsensusContext::MakeRecoveryMessage()
    {
        ...
        try {
            ...
            LOG_DEBUG("Prepared recovery message for view {} with {} change views, {} prepare reqs, {} prepare resps, {} commits",
    ...
    std::shared_ptr<ledger::Block> ConsensusContext::EnsureHeader()
    {
        ...
        try {
            ...
            LOG_DEBUG("Calculated merkle root for {} transactions: {}", 
                     hashes.size(), merkle_root.ToString());
    ...
    void ConsensusContext::Save()
    {
        ...
        try {
            ...
            if (Snapshot) {
                ...
                LOG_DEBUG("Saved consensus state for block {} view {} ({} bytes)", 
                         Block->GetIndex(), ViewNumber, serialized_data.Size());
            } else {
    ...
    void ConsensusContext::Serialize(io::BinaryWriter& writer) const
    {
        ...
        try {
            ...
            LOG_DEBUG("Serialized consensus context for block {} view {}", 
                     Block->GetIndex(), ViewNumber);
        ...
        try {
            ...
            for (size_t i = 0; i < prepCount; i++) {
                ...
                if (hasPayload) {
                    ...
                    LOG_DEBUG("Restored complete preparation payload for validator {} at index {}", 
                             payloadValidatorIndex, i);
                }
            }
            ...
            for (size_t i = 0; i < commitCount; i++) {
                ...
                if (hasPayload) {
                    ...
                    LOG_DEBUG("Restored complete commit payload for validator {} at index {}", 
                             payloadValidatorIndex, i);
                }
            }
            ...
            for (size_t i = 0; i < changeViewCount; i++) {
                ...
                if (hasPayload) {
                    ...
                    LOG_DEBUG("Restored complete change view payload for validator {} at index {}", 
                             payloadValidatorIndex, i);
                }
            }
            ...
            LOG_INFO("Deserialized consensus context for block {} view {} with {} validators", 
                    blockIndex, ViewNumber, validatorCount);
