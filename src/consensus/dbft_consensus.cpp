...
namespace neo::consensus
{
          ...
          logger_(core::Logger::GetInstance())
    {
        ...
        LOG_INFO("dBFT consensus initialized with {} validators, node index: {}", 
                validators_.size(), validator_index_);
    }
    ...
    bool DbftConsensus::ProcessMessage(const ConsensusMessage& message)
    {
        ...
        {
            LOG_DEBUG("Ignoring message for block height {} (current: {})", 
                     message.GetBlockIndex(), current_block_index);
            return false;
        }
        ...
        {
            LOG_WARNING("Invalid validator index {} in message (max: {})", 
                       validator_index, validators_.size() - 1);
            return false;
        }
        ...
        {
            LOG_DEBUG("Ignoring message for old view {} (current: {})", 
                     message.GetViewNumber(), current_view);
            return false;
        }
    ...
    void DbftConsensus::ProcessCommit(const CommitMessage& message)
    {
        ...
        try {
            ...
            if (!signature_valid) {
                LOG_WARNING("Invalid signature from validator {} for block hash {}", 
                           message.GetValidatorIndex(), block_hash.ToString());
                return;
            }
    ...
    bool DbftConsensus::VerifyBlock(const std::shared_ptr<ledger::Block>& block)
    {
        ...
        try {
            ...
            if (block->GetIndex() != expected_index) {
                LOG_WARNING("Block index {} does not match expected {}", 
                           block->GetIndex(), expected_index);
                return false;
            }
            ...
            if (time_diff.count() < -60 || time_diff.count() > 3600) { // 1 minute future, 1 hour past
    ...
    io::UInt256 DbftConsensus::CalculateMerkleRoot(const std::vector<network::p2p::payloads::Neo3Transaction>& transactions)
    {
        ...
        if (transactions.size() > config_.max_transactions_per_block) {
            LOG_ERROR("Transaction count {} exceeds maximum {}", 
                     transactions.size(), config_.max_transactions_per_block);
            return io::UInt256::Zero();
        }
