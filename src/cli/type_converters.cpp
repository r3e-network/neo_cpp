...
TypeConverters::TypeConverters() { InitializeDefaultConverters(); }
...
const std::unordered_map<std::string, TypeConverter>& TypeConverters::GetAllConverters() const { return converters_; }
...
void TypeConverters::InitializeDefaultConverters()
{
    ...
    RegisterConverter(
        ...
        [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
        {
            try
            {
                if (args.empty()) return new std::string("");
    ...
    RegisterConverter(
        ...
        [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
        {
            ...
            if (args.empty()) throw std::runtime_error("Missing argument for UInt256");
    ...
    RegisterConverter(
        ...
        {
            (void)canConsumeAll;  // Suppress unused parameter warning
            if (args.empty()) throw std::runtime_error("Missing argument for UInt160");
    ...
    RegisterConverter("ECPoint",
                      [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
                      {
                          (void)canConsumeAll;  // Suppress unused parameter warning
                          if (args.empty()) throw std::runtime_error("Missing argument for ECPoint");
    ...
    RegisterConverter("int",
                      ...
                      {
                          (void)canConsumeAll;  // Suppress unused parameter warning
                          if (args.empty()) throw std::runtime_error("Missing argument for int");

                          return new int(std::stoi(args[0]));
                      });

    // Register uint32_t converter
    RegisterConverter("uint32_t",
                      [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
                      {
                          (void)canConsumeAll;  // Suppress unused parameter warning
                          if (args.empty()) throw std::runtime_error("Missing argument for uint32_t");
    ...
    RegisterConverter("bool",
                      ...
                      {
                          (void)canConsumeAll;  // Suppress unused parameter warning
                          if (args.empty()) throw std::runtime_error("Missing argument for bool");
