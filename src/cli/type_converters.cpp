...
void TypeConverters::InitializeDefaultConverters()
{
    ...
    RegisterConverter("string",
                      [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
                      {
                          ...
                          {
                              throw std::runtime_error("Memory allocation failed for string conversion: " +
                                                       std::string(e.what()));
                          }
                      });

    // Register UInt256 converter - WARNING: Returns raw pointer, caller must delete
    RegisterConverter("UInt256",
                      [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
                      {
                          ...
                          {
                              throw std::runtime_error("Memory allocation failed for UInt256 conversion: " +
                                                       std::string(e.what()));
                          }
    ...
    RegisterConverter("UInt160",
                      [](const std::vector<std::string>& args, bool canConsumeAll) -> void*
                      {
                          ...
                          {
                              throw std::runtime_error("Memory allocation failed for UInt160 conversion: " +
                                                       std::string(e.what()));
                          }
                          catch (const std::exception& e)
                          {
