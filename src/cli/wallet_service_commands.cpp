...
namespace neo::cli
{
WalletCommands::WalletCommands(MainService& service) : service_(service) {}

void WalletCommands::RegisterCommands()
{
    service_.RegisterCommand(
        "openwallet", [this](const std::vector<std::string>& args) { return HandleOpenWallet(args); }, "Wallet");

    service_.RegisterCommand(
        "closewallet", [this](const std::vector<std::string>& args) { return HandleCloseWallet(args); }, "Wallet");

    service_.RegisterCommand(
        "showbalance", [this](const std::vector<std::string>& args) { return HandleShowBalance(args); }, "Wallet");

    service_.RegisterCommand(
        "showaddress", [this](const std::vector<std::string>& args) { return HandleShowAddress(args); }, "Wallet");

    service_.RegisterCommand(
        "transfer", [this](const std::vector<std::string>& args) { return HandleTransfer(args); }, "Wallet");
}
