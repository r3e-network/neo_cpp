...
namespace neo::cli
{
    BaseCommands::BaseCommands(MainService& service)
        : service_(service)
    {
    }
    ...
    {
        service_.RegisterCommand("help", [this](const std::vector<std::string>& args) {
            return HandleHelp(args);
        }, "Base");

        service_.RegisterCommand("exit", [this](const std::vector<std::string>& args) {
            return HandleExit(args);
        }, "Base");

        service_.RegisterCommand("clear", [this](const std::vector<std::string>& args) {
            return HandleClear(args);
        }, "Base");

        service_.RegisterCommand("version", [this](const std::vector<std::string>& args) {
