...
namespace neo::cli
{
    BlockchainCommands::BlockchainCommands(MainService& service)
        : service_(service)
    {
    }
    ...
    {
        service_.RegisterCommand("showblock", [this](const std::vector<std::string>& args) {
            return HandleShowBlock(args);
        }, "Blockchain");

        service_.RegisterCommand("showheader", [this](const std::vector<std::string>& args) {
            return HandleShowHeader(args);
        }, "Blockchain");

        service_.RegisterCommand("showtx", [this](const std::vector<std::string>& args) {
            return HandleShowTransaction(args);
        }, "Blockchain");
    }
