...
namespace neo::node
{
    ...
    bool NeoNode::InitializeNetwork()
    {
        ...
        {
            // Create P2P server
            auto listenEndpoint = network::IPEndPoint(
                network::IPAddress::Any(), 
                protocolSettings_->GetP2PPort()
            );
            ...
            peerDiscovery_ = std::make_shared<network::PeerDiscoveryService>(
                protocolSettings_,
                p2pServer_
            );
    ...
    bool NeoNode::InitializeRPC()
    {
        try
        {
            ...
            {
                rpcServer_ = std::make_shared<rpc::RpcServer>(
                    protocolSettings_->GetRpcPort(),
                    blockchain_,
                    memoryPool_,
                    applicationEngine_
                );
                
                logger_->Info("RPC server initialized on port {}", 
                            protocolSettings_->GetRpcPort());
            }
            else
            {
    ...
    bool NeoNode::InitializeConsensus()
    {
        try
        {
            ...
            {
                consensusService_ = std::make_shared<consensus::ConsensusService>(
                    protocolSettings_,
                    blockchain_,
                    memoryPool_,
                    p2pServer_
