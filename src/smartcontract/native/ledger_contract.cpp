...
LedgerContract::LedgerContract() : NativeContract(NAME, ID), currentBlockKey_(CreateStorageKey(PREFIX_CURRENT_BLOCK)) {}

void LedgerContract::Initialize()
{
    RegisterMethod("getHash", CallFlags::ReadStates,
                   std::bind(&LedgerContract::OnGetHash, this, std::placeholders::_1, std::placeholders::_2));
    RegisterMethod("getBlock", CallFlags::ReadStates,
                   std::bind(&LedgerContract::OnGetBlock, this, std::placeholders::_1, std::placeholders::_2));
    RegisterMethod("getTransaction", CallFlags::ReadStates,
                   std::bind(&LedgerContract::OnGetTransaction, this, std::placeholders::_1, std::placeholders::_2));
    RegisterMethod(
        "getTransactionHeight", CallFlags::ReadStates,
        std::bind(&LedgerContract::OnGetTransactionHeight, this, std::placeholders::_1, std::placeholders::_2));
    RegisterMethod("getCurrentIndex", CallFlags::ReadStates,
                   std::bind(&LedgerContract::OnGetCurrentIndex, this, std::placeholders::_1, std::placeholders::_2));
    RegisterMethod("getCurrentHash", CallFlags::ReadStates,
                   std::bind(&LedgerContract::OnGetCurrentHash, this, std::placeholders::_1, std::placeholders::_2));
}
...
std::shared_ptr<vm::StackItem> LedgerContract::OnGetCurrentHash(ApplicationEngine& engine,
                                                                const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    // Get current hash
...
std::shared_ptr<vm::StackItem> LedgerContract::TransactionToStackItem(std::shared_ptr<ledger::Transaction> tx) const
{
    ...
    auto result = vm::StackItem::Create(std::vector<std::shared_ptr<vm::StackItem>>{
        vm::StackItem::Create(tx->GetHash()), vm::StackItem::Create(static_cast<int64_t>(tx->GetVersion())),
        vm::StackItem::Create(static_cast<int64_t>(tx->GetNonce())), vm::StackItem::Create(tx->GetSender()),
        vm::StackItem::Create(static_cast<int64_t>(tx->GetSystemFee())),
        vm::StackItem::Create(static_cast<int64_t>(tx->GetNetworkFee())),
        vm::StackItem::Create(static_cast<int64_t>(tx->GetValidUntilBlock())), vm::StackItem::Create(tx->GetScript()),
...
bool LedgerContract::OnPersist(ApplicationEngine& engine)
{
    ...
    auto blockHashItem =
        persistence::StorageItem(io::ByteVector(io::ByteSpan(block->GetHash().Data(), io::UInt256::Size)));
    engine.GetSnapshot()->Add(blockHashKey, blockHashItem);

    // Store block
    auto blockKey =
        CreateStorageKey(PREFIX_BLOCK, io::ByteVector(io::ByteSpan(block->GetHash().Data(), io::UInt256::Size)));
    ...
    auto blockItem = persistence::StorageItem(
        io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(blockData.data()), blockData.size())));
    ...
    {
        auto txKey =
            CreateStorageKey(PREFIX_TRANSACTION, io::ByteVector(io::ByteSpan(tx.GetHash().Data(), io::UInt256::Size)));
        ...
        auto txItem = persistence::StorageItem(
            io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(txData.data()), txData.size())));
...
bool LedgerContract::PostPersist(ApplicationEngine& engine)
{
    ...
    auto item = engine.GetSnapshot()->GetAndChange(
        currentBlockKey_, []() { return std::make_shared<persistence::StorageItem>(io::ByteVector{}); });
