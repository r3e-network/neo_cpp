...
bool NeoToken::Transfer(ApplicationEngine& engine, std::shared_ptr<persistence::DataCache> snapshot,
                        const io::UInt160& from, const io::UInt160& to, const io::Fixed8& amount)
{
...
bool NeoToken::RegisterCandidate(std::shared_ptr<persistence::DataCache> snapshot,
                                 const cryptography::ecc::ECPoint& pubKey)
{
    return NeoTokenCandidate::RegisterCandidate(*this, snapshot, pubKey);
}

bool NeoToken::UnregisterCandidate(std::shared_ptr<persistence::DataCache> snapshot,
                                   const cryptography::ecc::ECPoint& pubKey)
{
    return NeoTokenCandidate::UnregisterCandidate(*this, snapshot, pubKey);
}

bool NeoToken::Vote(std::shared_ptr<persistence::DataCache> snapshot, const io::UInt160& account,
                    const std::vector<cryptography::ecc::ECPoint>& pubKeys)
{
    return NeoTokenVote::Vote(*this, snapshot, account, pubKeys);
}

std::shared_ptr<vm::StackItem> NeoToken::OnTotalSupply(neo::smartcontract::ApplicationEngine& engine,
                                                       const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenTransfer::OnTotalSupply(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnBalanceOf(neo::smartcontract::ApplicationEngine& engine,
                                                     const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenAccount::OnBalanceOf(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnTransfer(neo::smartcontract::ApplicationEngine& engine,
                                                    const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenTransfer::OnTransfer(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetValidators(neo::smartcontract::ApplicationEngine& engine,
                                                         const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCommittee::OnGetValidators(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetCommittee(neo::smartcontract::ApplicationEngine& engine,
                                                        const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCommittee::OnGetCommittee(*this, engine, args);
}
...
std::shared_ptr<vm::StackItem> NeoToken::OnRegisterCandidate(neo::smartcontract::ApplicationEngine& engine,
                                                             const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnRegisterCandidate(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnUnregisterCandidate(neo::smartcontract::ApplicationEngine& engine,
                                                               const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnUnregisterCandidate(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnVote(neo::smartcontract::ApplicationEngine& engine,
                                                const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenVote::OnVote(*this, engine, args);
}
...
NeoToken::GasDistribution NeoToken::DistributeGas(neo::smartcontract::ApplicationEngine& engine,
                                                  const io::UInt160& account, const AccountState& state)
{
    return NeoTokenGas::DistributeGas(*this, engine, account, state);
}

int64_t NeoToken::GetUnclaimedGas(std::shared_ptr<persistence::DataCache> snapshot, const io::UInt160& account,
                                  uint32_t end) const
{
    return NeoTokenGas::GetUnclaimedGas(*this, snapshot, account, end);
}

int64_t NeoToken::CalculateBonus(std::shared_ptr<persistence::DataCache> snapshot, const AccountState& state,
                                 uint32_t end) const
{
    return NeoTokenGas::CalculateBonus(*this, snapshot, state, end);
}

int64_t NeoToken::CalculateNeoHolderReward(std::shared_ptr<persistence::DataCache> snapshot, int64_t value,
...
std::shared_ptr<vm::StackItem> NeoToken::OnGetGasPerBlock(neo::smartcontract::ApplicationEngine& engine,
                                                          const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenGas::OnGetGasPerBlock(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnSetGasPerBlock(neo::smartcontract::ApplicationEngine& engine,
                                                          const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenGas::OnSetGasPerBlock(*this, engine, args);
}

NeoToken::AccountState NeoToken::GetAccountState(std::shared_ptr<persistence::DataCache> snapshot,
                                                 const io::UInt160& account) const
{
    return NeoTokenAccount::GetAccountState(*this, snapshot, account);
}

NeoToken::CandidateState NeoToken::GetCandidateState(std::shared_ptr<persistence::DataCache> snapshot,
...
int64_t NeoToken::GetCandidateVote(std::shared_ptr<persistence::DataCache> snapshot,
                                   const cryptography::ecc::ECPoint& pubKey) const
{
    return NeoTokenCandidate::GetCandidateVote(*this, snapshot, pubKey);
}

void NeoToken::CheckCandidate(std::shared_ptr<persistence::DataCache> snapshot,
                              const cryptography::ecc::ECPoint& pubKey, const CandidateState& state)
{
    NeoTokenCandidate::CheckCandidate(*this, snapshot, pubKey, state);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetAccountState(neo::smartcontract::ApplicationEngine& engine,
                                                           const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenAccount::OnGetAccountState(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetCandidates(neo::smartcontract::ApplicationEngine& engine,
                                                         const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnGetCandidates(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetCandidateVote(neo::smartcontract::ApplicationEngine& engine,
                                                            const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnGetCandidateVote(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetRegisterPrice(neo::smartcontract::ApplicationEngine& engine,
                                                            const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnGetRegisterPrice(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnSetRegisterPrice(neo::smartcontract::ApplicationEngine& engine,
                                                            const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenCandidate::OnSetRegisterPrice(*this, engine, args);
}

std::shared_ptr<vm::StackItem> NeoToken::OnGetUnclaimedGas(neo::smartcontract::ApplicationEngine& engine,
                                                           const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    return NeoTokenGas::OnGetUnclaimedGas(*this, engine, args);
}
