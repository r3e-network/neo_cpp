...
namespace neo::smartcontract::native
{
    ...
    {
        RegisterMethod("serialize", CallFlags::None, std::bind(&StdLib::OnSerialize, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("deserialize", CallFlags::None, std::bind(&StdLib::OnDeserialize, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("jsonSerialize", CallFlags::None, std::bind(&StdLib::OnJsonSerialize, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("jsonDeserialize", CallFlags::None, std::bind(&StdLib::OnJsonDeserialize, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("itoa", CallFlags::None, std::bind(&StdLib::OnItoa, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("atoi", CallFlags::None, std::bind(&StdLib::OnAtoi, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("base64Encode", CallFlags::None, std::bind(&StdLib::OnBase64Encode, this, std::placeholders::_1, std::placeholders::_2));
        ...
        RegisterMethod("stringCompare", CallFlags::None, std::bind(&StdLib::OnStringCompare, this, std::placeholders::_1, std::placeholders::_2));
        RegisterMethod("strLen", CallFlags::None, std::bind(&StdLib::OnStrLen, this, std::placeholders::_1, std::placeholders::_2));
    }

    std::shared_ptr<vm::StackItem> StdLib::OnSerialize(ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.empty())
        ...
        try
        {
            auto serialized = BinarySerializer::Serialize(item, engine.GetLimits().MaxItemSize, engine.GetLimits().MaxStackSize);
    ...
    std::shared_ptr<vm::StackItem> StdLib::OnDeserialize(ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.empty())
            throw std::runtime_error("Invalid arguments");
        ...
        try
        {
            return BinarySerializer::Deserialize(data.AsSpan(), engine.GetLimits().MaxItemSize, engine.GetLimits().MaxStackSize);
        }
        catch (const std::exception& e)
        {
    ...
    std::shared_ptr<vm::StackItem> StdLib::OnJsonSerialize(ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.empty())
            throw std::runtime_error("Invalid arguments");
    ...
    std::shared_ptr<vm::StackItem> StdLib::OnJsonDeserialize(ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.empty())
            throw std::runtime_error("Invalid arguments");
        ...
        // Use JsonSerializer exactly like C# JsonSerializer.Deserialize(engine, JToken.Parse(json, 10), engine.Limits, engine.ReferenceCounter)
        try
        {
            return JsonSerializer::Deserialize(data.AsSpan(), engine.GetLimits().MaxItemSize, engine.GetLimits().MaxStackSize);
        }
