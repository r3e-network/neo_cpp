...
const io::UInt256& HashIndexState::GetHash() const { return hash_; }

void HashIndexState::SetHash(const io::UInt256& hash) { hash_ = hash; }

uint32_t HashIndexState::GetIndex() const { return index_; }

void HashIndexState::SetIndex(uint32_t index) { index_ = index; }
...
void HashIndexState::FromStackItem(const std::shared_ptr<vm::StackItem>& item)
{
    ...
    if (structItem->Count() < 2) throw std::runtime_error("Invalid stack item count");

    auto hashBytes = structItem->Get(0)->GetByteArray();
    if (hashBytes.Size() != 32) throw std::runtime_error("Invalid hash size");
