...
bool NeoTokenTransfer::Transfer(const NeoToken& token, ApplicationEngine& engine,
                                ...
                                const io::UInt160& to, const io::Fixed8& amount)
{
    ...
    if (from != io::UInt160())
    {
        ...
        persistence::StorageKey fromKey =
            token.CreateStorageKey(static_cast<uint8_t>(NeoToken::StoragePrefix::Account),
                                   io::ByteVector(io::ByteSpan(from.Data(), io::UInt160::Size)));
        persistence::StorageItem fromItem(
            io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(fromData.data()), fromData.size())));
        ...
        if (!fromState.voteTo.IsInfinity())
        {
            ...
            if (candidateState.registered)
            {
                ...
                persistence::StorageKey candidateKey = token.CreateStorageKey(
                    static_cast<uint8_t>(NeoToken::StoragePrefix::Candidate), fromState.voteTo.ToArray());
                persistence::StorageItem candidateItem(io::ByteVector(
                    io::ByteSpan(reinterpret_cast<const uint8_t*>(candidateData.data()), candidateData.size())));
    ...
    if (to != io::UInt160())
    {
        ...
        persistence::StorageKey toKey =
            token.CreateStorageKey(static_cast<uint8_t>(NeoToken::StoragePrefix::Account),
                                   io::ByteVector(io::ByteSpan(to.Data(), io::UInt160::Size)));
        persistence::StorageItem toItem(
            io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(toData.data()), toData.size())));
        ...
        if (!toState.voteTo.IsInfinity())
        {
            ...
            if (candidateState.registered)
            {
                ...
                persistence::StorageKey candidateKey = token.CreateStorageKey(
                    static_cast<uint8_t>(NeoToken::StoragePrefix::Candidate), toState.voteTo.ToArray());
                persistence::StorageItem candidateItem(io::ByteVector(
                    io::ByteSpan(reinterpret_cast<const uint8_t*>(candidateData.data()), candidateData.size())));
                snapshot->Add(candidateKey, candidateItem);
            }
...
std::shared_ptr<vm::StackItem> NeoTokenTransfer::OnTotalSupply(const NeoToken& token, ApplicationEngine& engine,
                                                            ...
                                                            const std::vector<std::shared_ptr<vm::StackItem>>& args)
{
    ...
    std::vector<std::shared_ptr<vm::StackItem>> state = {
        ...
        to == io::UInt160() ? vm::StackItem::Null() : vm::StackItem::Create(to), vm::StackItem::Create(amount.Value())};
