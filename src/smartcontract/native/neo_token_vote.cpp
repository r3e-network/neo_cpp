...
{
    bool NeoTokenVote::Vote(const NeoToken& token, std::shared_ptr<persistence::DataCache> snapshot, const io::UInt160& account, const std::vector<cryptography::ecc::ECPoint>& pubKeys)
    {
        ...
        else
        {
            ...
            persistence::StorageKey gasKey = token.CreateStorageKey(static_cast<uint8_t>(NeoToken::StoragePrefix::VoterReward), pubKeys[0].ToArray());
            auto gasItem = snapshot->TryGet(gasKey);
            if (gasItem)
            {
                std::istringstream gasStream(std::string(reinterpret_cast<const char*>(gasItem->GetValue().Data()), gasItem->GetValue().Size()));
    ...
    std::shared_ptr<vm::StackItem> NeoTokenVote::OnVote(const NeoToken& token, ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.size() < 2)
