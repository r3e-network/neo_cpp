...
{
    io::Fixed8 NeoTokenAccount::GetBalance(const NeoToken& token, std::shared_ptr<persistence::DataCache> snapshot, const io::UInt160& account)
    {
        auto state = GetAccountState(token, snapshot, account);
        return io::Fixed8(state.balance);
    }

    NeoToken::AccountState NeoTokenAccount::GetAccountState(const NeoToken& token, std::shared_ptr<persistence::DataCache> snapshot, const io::UInt160& account)
    {
        persistence::StorageKey key = token.CreateStorageKey(static_cast<uint8_t>(NeoToken::StoragePrefix::Account), account);
    ...
    std::shared_ptr<vm::StackItem> NeoTokenAccount::OnBalanceOf(const NeoToken& token, ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.size() < 1)
    ...
    std::shared_ptr<vm::StackItem> NeoTokenAccount::OnGetAccountState(const NeoToken& token, ApplicationEngine& engine, const std::vector<std::shared_ptr<vm::StackItem>>& args)
    {
        if (args.size() < 1)
            throw std::runtime_error("Invalid number of arguments");
        ...
        stateItems.push_back(state.voteTo.IsInfinity() ? vm::StackItem::Null() : vm::StackItem::Create(state.voteTo.ToArray()));
