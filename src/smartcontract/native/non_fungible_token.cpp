...
int64_t NonFungibleToken::GetBalanceOf(std::shared_ptr<persistence::StoreView> snapshot,
...
io::UInt160 NonFungibleToken::GetOwnerOf(std::shared_ptr<persistence::StoreView> snapshot,
...
std::vector<io::ByteVector> NonFungibleToken::GetTokensOf(std::shared_ptr<persistence::StoreView> snapshot,
                                                          const io::UInt160& account) const
{
    std::vector<io::ByteVector> tokens;
...
bool NonFungibleToken::Transfer(std::shared_ptr<persistence::StoreView> snapshot, const io::UInt160& from,
                                const io::UInt160& to, const io::ByteVector& tokenId)
{
    ...
    if (newFromBalance > 0)
    {
        io::ByteVector fromBalanceValue(
            io::ByteSpan(reinterpret_cast<const uint8_t*>(&newFromBalance), sizeof(int64_t)));
...
bool NonFungibleToken::Transfer(ApplicationEngine& engine, const io::UInt160& from, const io::UInt160& to,
...
bool NonFungibleToken::Mint(std::shared_ptr<persistence::StoreView> snapshot, const io::ByteVector& tokenId,
...
bool NonFungibleToken::Mint(ApplicationEngine& engine, const io::ByteVector& tokenId, const io::UInt160& owner,
                            const std::map<std::string, std::shared_ptr<vm::StackItem>>& properties,
...
bool NonFungibleToken::PostTransfer(ApplicationEngine& engine, const io::UInt160& from, const io::UInt160& to,
                                    int64_t amount, const io::ByteVector& tokenId, std::shared_ptr<vm::StackItem> data,
                                    bool callOnPayment)
{
