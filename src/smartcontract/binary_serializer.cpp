...
namespace neo::smartcontract
{
    ...
    void BinarySerializer::SerializeStackItem(io::BinaryWriter& writer, std::shared_ptr<vm::StackItem> item, int64_t maxSize, int64_t maxItems, int64_t& itemCount)
    {
    ...
    std::shared_ptr<vm::StackItem> BinarySerializer::DeserializeStackItem(io::BinaryReader& reader, int64_t maxSize, int64_t maxItems, int64_t& itemCount)
    {
        if (++itemCount > maxItems)
            throw std::runtime_error("Maximum item count exceeded");
