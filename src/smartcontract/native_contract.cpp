...
namespace neo::smartcontract
{
    ...
    NativeContract::NativeContract(const std::string& name, int32_t id)
        : name_(name), id_(id)
    {
        // Calculate script hash
        std::string script = "neo.native." + name;
        scriptHash_ = cryptography::Hash::Hash160(io::ByteSpan(reinterpret_cast<const uint8_t*>(script.data()), script.size()));
        
        // Create contract state
        contractState_.SetId(id);
        contractState_.SetScriptHash(scriptHash_);
        contractState_.SetScript(io::ByteVector(io::ByteSpan(reinterpret_cast<const uint8_t*>(script.data()), script.size())));
    ...
    void NativeContract::RegisterMethod(const std::string& name, std::function<bool(ApplicationEngine&)> handler, CallFlags flags)
    {
