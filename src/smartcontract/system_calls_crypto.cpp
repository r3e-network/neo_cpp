...
void RegisterCryptoSystemCallsImpl(ApplicationEngine& engine)
{
                ...
                if (auto neo3tx = dynamic_cast<const network::p2p::payloads::Neo3Transaction*>(container))
                {
                    ...
                    try
                    {
                        ...
                        for (const ledger::Signer& signerObj : signersList)
                        {
                            ...
                            if ((static_cast<uint8_t>(signerObj.GetScopes()) &
                                 static_cast<uint8_t>(ledger::WitnessScope::CustomContracts)) != 0)
                            {
                            ...
                            if ((static_cast<uint8_t>(signerObj.GetScopes()) &
                                 static_cast<uint8_t>(ledger::WitnessScope::CustomGroups)) != 0)
                            {
    ...
    engine.RegisterSystemCall(
        ...
        [](vm::ExecutionEngine& engine)
        {
            ...
            try
            {
                ...
                for (size_t sigIndex = 0; sigIndex < signaturesArray.size() && pubKeyIndex < pubKeysArray.size();
                     sigIndex++)
                {
    ...
    engine.RegisterSystemCall(
        "System.Crypto.VerifyBLS12381Signature",
        [](vm::ExecutionEngine& engine)
        {
            auto& appEngine = static_cast<ApplicationEngine&>(engine);
            auto& context = appEngine.GetCurrentContext();
...
io::ByteVector MapToG1SSWU(const std::array<uint8_t, 48>& u0, const std::array<uint8_t, 48>& u1)
{
    ...
    static const uint8_t FIELD_MODULUS[48] = {0x1a, 0x01, 0x11, 0xea, 0x39, 0x7f, 0xe6, 0x9a, 0x4b, 0x1b, 0xa7, 0xb6,
...
bool VerifyBLS12381Pairing(const io::ByteVector& signature, const io::ByteVector& pubkey,
                           const io::ByteVector& message_point)
{
    try
    {
...
void ApplyIsogenyMap(const uint64_t* x_prime, const uint64_t* y_prime, uint64_t* x_result, uint64_t* y_result)
{
    ...
    try {
        ...
        // x-coordinate map: x = (k_(1,0) + k_(1,1) * x' + k_(1,2) * x'^2 + k_(1,3) * x'^3) / (1 + k_(2,1) * x' +
k_(2,2) * x'^2) static const uint64_t K_1_0[6] = {0x11560bf17baa99bcULL, 0x89f550f47ba1fcaaULL, 0x824b15581a4c101bULL,
0x7714250171d6588eULL, 0x0, 0x0}; static const uint64_t K_1_1[6] = {0x3dd3a569412c0a34ULL, 0xa88a657291f1a540ULL,
0xa8c6df28d7b1c3e5ULL, 0x7f2ea6e43f99b5a7ULL, 0x0, 0x0}; static const uint64_t K_1_2[6] = {0x3dd3a569412c0a34ULL,
0xa88a657291f1a540ULL, 0xa8c6df28d7b1c3e5ULL, 0x7f2ea6e43f99b5a7ULL, 0x0, 0x0}; static const uint64_t K_1_3[6] = {1, 0,
0, 0, 0, 0};

        static const uint64_t K_2_1[6] = {0x5fe55555554c71d0ULL, 0x873fffdd236aaaaaULL, 0x6a6b4619b26ef918ULL,
0x21c2888408874945ULL, 0x0, 0x0}; static const uint64_t K_2_2[6] = {0xa1f0fac9f8000000ULL, 0x9a2ca556b9f76408ULL,
0x92202e0f6bff5f98ULL, 0xe2a777fffffffbULL, 0x0, 0x0};
