...
namespace neo::json
{
    ...
    JTokenType JObject::GetType() const
    {
    ...
    {
        if (other.GetType() != JTokenType::Object)
            return false;
        
        const auto& other_object = static_cast<const JObject&>(other);
        if (properties_.size() != other_object.properties_.size())
            return false;
        ...
        for (size_t i = 0; i < properties_.size(); ++i)
        {
            ...
            if (!other_object.properties_.contains(key))
                return false;
    ...
    const JObject::Properties& JObject::GetProperties() const
    {
        return properties_;
    }

    JObject::Properties& JObject::GetProperties()
    {
        return properties_;
    }

    bool JObject::ContainsProperty(const std::string& key) const
    {
