...
SecureStringExtensions::SecureString::SecureString(const std::string& str) : length_(str.length())
{
    if (length_ > 0)
    {
...
SecureStringExtensions::SecureString::SecureString(const char* str) : length_(str ? std::strlen(str) : 0)
{
    if (length_ > 0)
    {
        data_ = std::make_unique<char[]>(length_ + 1);
...
SecureStringExtensions::SecureString::SecureString(const char* data, size_t length) : length_(length)
{
    if (length_ > 0 && data)
    {
        data_ = std::make_unique<char[]>(length_ + 1);
...
bool SecureStringExtensions::ValidatePasswordStrength(const SecureString& password, size_t minLength,
                                                      ...
                                                      bool requireSpecial)
{
    ...
    return (!requireUppercase || hasUpper) && (!requireLowercase || hasLower) && (!requireDigits || hasDigit) &&
           (!requireSpecial || hasSpecial);
}

SecureStringExtensions::SecureString SecureStringExtensions::SecureConcat(const SecureString& left,
...
SecureStringExtensions::SecureString SecureStringExtensions::SecureHash(const SecureString& input,
                                                                        const SecureString& salt)
{
