...
void AssemblyExtensions::RegisterMethod(const std::string& typeName, const std::string& methodName,
                                        const std::string& signature, std::function<void*()> invoker)
{
    static std::mutex registryMutex;
    std::lock_guard<std::mutex> lock(registryMutex);
...
void* AssemblyExtensions::InvokeMethod(const std::string& typeName, const std::string& methodName,
