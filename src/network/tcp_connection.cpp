...
{
TcpConnection::TcpConnection(boost::asio::io_context& ioContext) : socket_(ioContext), running_(false), sending_(false)
{
}
...
void TcpConnection::ReadMessage()
{
    ...
    socket_.async_read_some(
        boost::asio::buffer(receiveBuffer_, 24),
        std::bind(&TcpConnection::HandleReadHeader, shared_from_this(), std::placeholders::_1, std::placeholders::_2));
}
...
void TcpConnection::HandleReadHeader(const std::error_code& error, size_t bytesTransferred)
{
    ...
    try
    {
        ...
        if (commandStr == "version")
            command = p2p::MessageCommand::Version;
        else if (commandStr == "verack")
            command = p2p::MessageCommand::Verack;
        else if (commandStr == "getaddr")
            command = p2p::MessageCommand::GetAddr;
        else if (commandStr == "addr")
            command = p2p::MessageCommand::Addr;
        else if (commandStr == "inv")
            command = p2p::MessageCommand::Inv;
        else if (commandStr == "getdata")
            command = p2p::MessageCommand::GetData;
        else if (commandStr == "getblocks")
            command = p2p::MessageCommand::GetBlocks;
        else if (commandStr == "getblockbyindex")
            command = p2p::MessageCommand::GetBlockByIndex;
        else if (commandStr == "getheaders")
            command = p2p::MessageCommand::GetHeaders;
        else if (commandStr == "tx")
            command = p2p::MessageCommand::Transaction;
        else if (commandStr == "block")
            command = p2p::MessageCommand::Block;
        else if (commandStr == "headers")
            command = p2p::MessageCommand::Headers;
        else if (commandStr == "ping")
            command = p2p::MessageCommand::Ping;
        else if (commandStr == "pong")
            command = p2p::MessageCommand::Pong;
        else if (commandStr == "merkleblock")
            command = p2p::MessageCommand::MerkleBlock;
        else if (commandStr == "filterload")
            command = p2p::MessageCommand::FilterLoad;
        else if (commandStr == "filteradd")
            command = p2p::MessageCommand::FilterAdd;
        else if (commandStr == "filterclear")
            command = p2p::MessageCommand::FilterClear;
        else if (commandStr == "reject")
            command = p2p::MessageCommand::Reject;
        else if (commandStr == "alert")
            command = p2p::MessageCommand::Alert;
        else if (commandStr == "extensible")
            command = p2p::MessageCommand::Extensible;
        else
            command = p2p::MessageCommand::Unknown;
                                ...
                                p2p::MessageFlags flags)
{
    ...
    boost::asio::async_read(socket_, boost::asio::buffer(receiveBuffer_, payloadLength),
                            std::bind(&TcpConnection::HandleReadPayload, shared_from_this(), std::placeholders::_1,
                                      std::placeholders::_2, command, payloadLength, checksum, flags));
}

void TcpConnection::HandleReadPayload(const std::error_code& error, size_t bytesTransferred,
                                      p2p::MessageCommand command, uint32_t expectedLength, uint32_t checksum,
                                      p2p::MessageFlags flags)
{
