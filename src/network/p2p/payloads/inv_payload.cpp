...
{
    InvPayload::InvPayload()
        : type_(InventoryType::Transaction)
    {
    }

    InvPayload::InvPayload(InventoryType type, const std::vector<io::UInt256>& hashes)
    ...
    void InvPayload::Deserialize(io::BinaryReader& reader)
    {
        ...
        if (type_ != InventoryType::Transaction &&
            type_ != InventoryType::Block &&
            type_ != InventoryType::Consensus)
        {
