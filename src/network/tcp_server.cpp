...
namespace neo::network
{
                ...
                ioThreads_.emplace_back([this, i]() {
                    ...
                    catch (const std::exception& e)
                    {
                        ...
                        logging::Logger::Instance().Error("Network",
                            std::string("Error in IO thread ") + std::to_string(i) + ": " + e.what());
                    }
                });
            }
    ...
    void TcpServer::HandleAccept(std::shared_ptr<TcpConnection> connection, const std::error_code& error)
    {
        ...
        try {
            ...
            logging::Logger::Instance().Warning("Network",
                std::string("Error getting remote endpoint: ") + e.what());
            connection->Close();
            return;
        }
        ...
        // Check if we have reached the connection limit
        {
            ...
            {
                logging::Logger::Instance().Warning("Network",
                    "Connection limit reached, rejecting connection from " + remoteEndpoint.ToString());
                connection->Close();
                return;
            }
