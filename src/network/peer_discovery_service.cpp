...
PeerDiscoveryService::PeerDiscoveryService(boost::asio::io_context& ioContext, 
...
void PeerDiscoveryService::AttemptConnections()
{
    ...
    {
        neo::logging::Logger::Instance().Debug("P2P", "Maximum peer count reached: " + 
                                              std::to_string(currentPeerCount) + "/" + 
                                              std::to_string(maxPeers_));
        return;
    }
    ...
    if (currentPeerCount < 3 && !seedNodes_.empty())
    {
        for (const auto& seed : seedNodes_)
        {
            ...
            {
                neo::logging::Logger::Instance().Error("P2P", "Failed to connect to seed node " + endpoint + 
                                                      ": " + std::string(ex.what()));
            }
        }
    }
...
void PeerDiscoveryService::CleanupOldPeers()
{
    ...
    while (it != knownPeers_.end())
    {
        ...
        bool isSeed = std::find_if(seedNodes_.begin(), seedNodes_.end(),
            [&it](const NetworkAddress& addr)
            {
