...
namespace neo::network
{
        ...
        try
        {
            ...
            while (std::chrono::steady_clock::now() - start < timeOut_)
            {
                ...
                catch (const std::exception& e)
                {
                    ...
                    logging::Logger::Instance().Warning("Network", std::string("Error receiving UPnP response: ") + e.what());
                }
            }
    ...
    void UPnP::ForwardPort(uint16_t port, const std::string& protocol, const std::string& description)
    {
        ...
        std::string soap = "<u:AddPortMapping xmlns:u=\"urn:schemas-upnp-org:service:WANIPConnection:1\">"
                          ...
                          "<NewInternalClient>" + local_address.to_string() + "</NewInternalClient>"
        ...
        logging::Logger::Instance().Info("Network", "UPnP port forwarding created for " + protocol + " port " + std::to_string(port));
    }
    ...
    void UPnP::DeleteForwardingRule(uint16_t port, const std::string& protocol)
    {
        ...
        logging::Logger::Instance().Info("Network", "UPnP port forwarding deleted for " + protocol + " port " + std::to_string(port));
    }
    ...
    std::string UPnP::SOAPRequest(const std::string& url, const std::string& soap, const std::string& function)
    {
        try
        {
            ...
            // Send the HTTP request
            boost::beast::http::request<boost::beast::http::string_body> http_req{boost::beast::http::verb::post, target, 11};
