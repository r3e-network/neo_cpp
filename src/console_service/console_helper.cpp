...
namespace neo::console_service
{
    ...
    ConsoleColorSet::ConsoleColorSet()
        : foreground_(ConsoleHelper::GetCurrentForegroundColor())
        , background_(ConsoleHelper::GetCurrentBackgroundColor())
    {
    }
    ...
    {
        const std::string valid_chars = " !\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
        std::string input;
