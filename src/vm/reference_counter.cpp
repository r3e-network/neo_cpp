...
namespace neo::vm
{
    ...
    bool ReferenceCounter::NeedTrack(std::shared_ptr<StackItem> item)
    {
        ...
        if (item->GetType() == StackItemType::Array ||
            item->GetType() == StackItemType::Struct ||
            item->GetType() == StackItemType::Map ||
            item->GetType() == StackItemType::Buffer)
            return true;
    ...
    void ReferenceCounter::FindStronglyConnectedComponents(std::vector<std::unordered_set<std::shared_ptr<StackItem>>>& components)
    {
    ...
    void ReferenceCounter::StrongConnect(
    ...
    bool ReferenceCounter::HasCircularReference(std::shared_ptr<StackItem> root, std::unordered_set<std::shared_ptr<StackItem>>& visited) const
    {
        if (!root)
            return false;
