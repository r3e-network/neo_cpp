...
int Instruction::Size() const
{
    ...
    return prefixSize > 0 ? 1 + prefixSize + Operand.Size() : 1 + OperandSizeTable[static_cast<uint8_t>(opcode)];
}
...
int64_t Instruction::TokenI64() const
{
    ...
    return static_cast<int64_t>(static_cast<uint64_t>(Operand[0]) | (static_cast<uint64_t>(Operand[1]) << 8) |
                                ...
                                (static_cast<uint64_t>(Operand[4]) << 32) | (static_cast<uint64_t>(Operand[5]) << 40) |
                                (static_cast<uint64_t>(Operand[6]) << 48) | (static_cast<uint64_t>(Operand[7]) << 56));
}
