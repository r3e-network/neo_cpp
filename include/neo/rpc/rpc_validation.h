...
public:
    ...
    static void ValidateParamCount(const io::json::JArray& params, size_t minCount, size_t maxCount) {
        ...
        if (count < minCount) {
            throw std::invalid_argument("Too few parameters (expected at least " + 
                                      std::to_string(minCount) + ", got " + 
                                      std::to_string(count) + ")");
        }
        if (count > maxCount) {
            throw std::invalid_argument("Too many parameters (expected at most " + 
     ...
     * @throws std::invalid_argument if invalid
     */
    static std::string GetStringParam(const io::json::JArray& params, size_t index, 
     ...
     * @throws std::invalid_argument if invalid
     */
    static int64_t GetIntParam(const io::json::JArray& params, size_t index,
                              const std::string& paramName) {
        if (index >= params.size()) {
            throw std::invalid_argument("Missing required parameter: " + paramName);
        }
     ...
     * @throws std::invalid_argument if invalid
     */
    static bool GetBoolParam(const io::json::JArray& params, size_t index,
                           const std::string& paramName) {
        if (index >= params.size()) {
            throw std::invalid_argument("Missing required parameter: " + paramName);
        }
    ...
    static std::string GetOptionalStringParam(const io::json::JArray& params, size_t index,
        ...
        if (param->GetType() != io::json::JType::String) {
            throw std::invalid_argument("Parameter at index " + std::to_string(index) + 
                                      " must be a string");
        }
    ...
    static void ValidateGasAmount(const std::string& gas) {
        try {
            ...
            if (amount > 10000000000) { // 10 billion GAS max
