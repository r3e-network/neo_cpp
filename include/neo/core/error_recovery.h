...
class ErrorRecovery
{
   public:
    ...
    struct RecoveryResult
    {
        ...
        RecoveryResult(const T& val, int attempts = 1) : success(true), value(val), attempts_made(attempts) {}
     ...
     */
    template <typename T>
    static RecoveryResult<T> WithFallback(std::function<T()> primary_operation, std::function<T()> fallback_operation)
    {
        try
        {
        ...
        catch (const NeoException& e)
        {
            ...
            catch (const std::exception& fallback_error)
            {
                return RecoveryResult<T>(
                    "Primary failed: " + std::string(e.what()) + ", Fallback failed: " + fallback_error.what(),
                    e.GetErrorCode(), 2);
            }
        }
        catch (const std::exception& e)
       ...
       public:
        ...
        RecoveryResult<T> Execute(std::function<T()> operation)
        {
            ...
            if (state_ == State::OPEN)
            {
                ...
                {
                    return RecoveryResult<T>("Circuit breaker is OPEN", NeoException::ErrorCode::TIMEOUT, 0);
                }
            }
    ...
    class Bulkhead
    {
       public:
        ...
        RecoveryResult<T> Execute(std::function<T()> operation)
        {
            ...
            {
                return RecoveryResult<T>("Bulkhead capacity exceeded", NeoException::ErrorCode::TIMEOUT, 0);
            }
     ...
     */
    template <typename T>
    static RecoveryResult<T> WithTimeout(std::function<T()> operation, std::chrono::milliseconds timeout)
    {
     ...
     */
    template <typename T>
    static RecoveryResult<T> SafeExecute(std::function<T()> operation, const std::string& operation_name = "operation")
    {
        try
        {
            T result = operation();
        ...
        {
            return RecoveryResult<T>(operation_name + " failed: " + e.what(), NeoException::ErrorCode::UNKNOWN_ERROR,
                                     1);
        }
        catch (...)
        {
...
#define WITH_FALLBACK(primary, fallback)                                                     \
    neo::core::ErrorRecovery::WithFallback<decltype(primary())>([&]() { return primary(); }, \
                                                                [&]() { return fallback(); })
