...
class Validator
{
   public:
    ...
    struct ValidationResult
    {
        ...
        ValidationResult(bool valid = true) : is_valid(valid), error_code(NeoException::ErrorCode::UNKNOWN_ERROR) {}
    ...
    static ValidationResult ValidatePositive(T value, const std::string& name = "value")
    {
        if (value <= 0)
        {
            return ValidationResult(false, name + " must be positive", NeoException::ErrorCode::INVALID_ARGUMENT);
        }
        return ValidationResult(true);
    }
    ...
    static ValidationResult ValidateNonNegative(T value, const std::string& name = "value")
    {
        if (value < 0)
        {
            return ValidationResult(false, name + " must be non-negative", NeoException::ErrorCode::INVALID_ARGUMENT);
        }
        return ValidationResult(true);
    }
    ...
    static ValidationResult ValidateSizeRange(size_t actual_size, size_t min_size, size_t max_size,
                                              const std::string& name = "data");

    // String validation
    static ValidationResult ValidateHexString(const std::string& hex, const std::string& name = "hex string");
    static ValidationResult ValidateHexString(const std::string& hex, size_t expected_length,
                                              const std::string& name = "hex string");
    ...
    static ValidationResult ValidateByteSpan(const io::ByteSpan& span, size_t expected_size,
                                             const std::string& name = "ByteSpan");

    // Container validation
    template <typename T>
    static ValidationResult ValidateContainer(const std::vector<T>& container, size_t max_size,
                                              const std::string& name = "container")
    {
        if (container.size() > max_size)
        {
            return ValidationResult(false,
                                    name + " size (" + std::to_string(container.size()) + ") exceeds maximum (" +
    ...
    template <typename T>
    static ValidationResult ValidateContainerNotEmpty(const std::vector<T>& container,
                                                      const std::string& name = "container")
    {
        if (container.empty())
        {
                                           ...
                                           const std::string& error_message, const std::string& name = "value")
    {
        if (!validator(value))
        {
            return ValidationResult(false, name + ": " + error_message, NeoException::ErrorCode::INVALID_ARGUMENT);
        }
        return ValidationResult(true);
    }
    ...
    static void RequireHexString(const std::string& hex, size_t expected_length,
                                 const std::string& name = "hex string");
    ...
    static ValidationResult ValidateBlockHeight(uint32_t height,
