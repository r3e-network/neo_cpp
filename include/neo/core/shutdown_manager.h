...
class ShutdownManager
{
  public:
     ...
     */
    void RegisterHandler(const std::string& name, ShutdownHandler handler, int priority = 100,
                         std::chrono::milliseconds timeout = std::chrono::milliseconds(30000))
    {
        ...
        std::sort(handlers_.begin(), handlers_.end(),
                  [](const HandlerInfo& a, const HandlerInfo& b) { return a.priority < b.priority; });
    }
    ...
    void InstallSignalHandlers()
    {
        ...
        signal(SIGINT, [](int sig) { ShutdownManager::GetInstance().RequestShutdown("SIGINT received"); });

        // Install SIGTERM handler
        signal(SIGTERM, [](int sig) { ShutdownManager::GetInstance().RequestShutdown("SIGTERM received"); });

#ifdef SIGHUP
        // Install SIGHUP handler (terminal hangup)
        signal(SIGHUP, [](int sig) { ShutdownManager::GetInstance().RequestShutdown("SIGHUP received"); });
#endif
    }
        ...
        for (const auto& info : handlers_)
        {
            ...
            std::thread handlerThread(
                [&]()
                {
                    ...
                    {
                        std::cerr << "\n  ❌ Handler '" << info.name << "' failed: " << e.what() << std::endl;
                    }
                });
            ...
            if (handlerThread.joinable())
            {
                if (!completed)
                {
                    std::cerr << "\n  ⚠️  Handler '" << info.name << "' timed out after " << info.timeout.count() << "ms"
                              << std::endl;
