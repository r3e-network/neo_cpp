...
class ECDSA
{
   public:
     ...
     * @return True if the signature is valid, false otherwise.
     */
    static bool VerifyHash(const io::UInt256& hash, const std::vector<uint8_t>& signature,
                           const std::vector<uint8_t>& publicKey);
     ...
     */
    static std::vector<uint8_t> Sign(const std::vector<uint8_t>& message, const std::vector<uint8_t>& privateKey);
     ...
     * @return The signature.
     */
    static std::vector<uint8_t> SignHash(const io::UInt256& hash, const std::vector<uint8_t>& privateKey);
     ...
     */
    static void GenerateKeyPair(std::vector<uint8_t>& privateKey, std::vector<uint8_t>& publicKey);
};
