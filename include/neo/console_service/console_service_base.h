...
namespace neo::console_service
{
    ...
    private:
         ...
         */
        bool TryProcessValue(const std::type_info& parameter_type,
                           std::vector<std::shared_ptr<CommandToken>>& args,
                           bool can_consume_all,
                           void*& value);
    ...
    template<typename T>
    void ConsoleServiceBase::RegisterCommandHandler(std::function<T(std::vector<std::shared_ptr<CommandToken>>&, bool)> handler)
    {
