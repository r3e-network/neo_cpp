...
namespace neo::console_service
{
    ...
    public:
         ...
         */
        static std::vector<std::string> ToArguments(const std::vector<std::shared_ptr<CommandToken>>& tokens, bool remove_escape = true);
    ...
    public:
         ...
         */
        static std::shared_ptr<CommandStringToken> Parse(const std::string& command_line, int& index, std::shared_ptr<CommandQuoteToken> quote = nullptr);
    };
}
