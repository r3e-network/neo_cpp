...
namespace neo::monitoring
{
    ...
    public:
        explicit DiskSpaceHealthCheck(const std::string& path,
                                    uint64_t min_free_gb = 10,
                                    uint64_t warning_free_gb = 50)
            : path_(path),
    ...
    public:
        explicit MemoryHealthCheck(double max_usage_percentage = 90.0,
                                 double warning_usage_percentage = 80.0)
            : max_usage_percentage_(max_usage_percentage),
              warning_usage_percentage_(warning_usage_percentage) {}
    ...
    public:
        explicit ConsensusHealthCheck(std::shared_ptr<consensus::DbftConsensus> consensus)
    ...
    public:
        explicit BlockchainSyncHealthCheck(std::shared_ptr<persistence::DataCache> db,
                                         std::shared_ptr<network::p2p::LocalNode> node,
                                         uint32_t max_blocks_behind = 10)
