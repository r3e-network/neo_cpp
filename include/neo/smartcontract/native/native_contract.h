...
namespace neo::smartcontract::native
{
    ...
    public:
         ...
         * @return The methods.
         */
        const std::unordered_map<std::string, std::pair<CallFlags, std::function<std::shared_ptr<vm::StackItem>(ApplicationEngine&, const std::vector<std::shared_ptr<vm::StackItem>>&)>>>& GetMethods() const;
         ...
         */
        void PutStorageValue(std::shared_ptr<persistence::StoreView> snapshot, const io::ByteVector& key, const io::ByteVector& value) const;
    ...
    protected:
         ...
         */
        void RegisterMethod(const std::string& name, CallFlags callFlags, std::function<std::shared_ptr<vm::StackItem>(ApplicationEngine&, const std::vector<std::shared_ptr<vm::StackItem>>&)> handler);

    private:
        std::string name_;
        uint32_t id_;
        io::UInt160 scriptHash_;
        std::unordered_map<std::string, std::pair<CallFlags, std::function<std::shared_ptr<vm::StackItem>(ApplicationEngine&, const std::vector<std::shared_ptr<vm::StackItem>>&)>>> methods_;
    };
}
