...
class ContractParametersContext
{
  ...
  public:
     ...
     */
    ContractParametersContext(const persistence::DataCache& snapshotCache,
                              const network::p2p::payloads::IVerifiable& verifiable, uint32_t network);
     ...
     */
    bool AddSignature(const Contract& contract, const cryptography::ecc::ECPoint& pubkey,
     ...
     */
    static std::unique_ptr<ContractParametersContext> FromJson(const io::JsonReader& reader,
                                                               const persistence::DataCache& snapshotCache);
