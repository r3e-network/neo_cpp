...
private:
    ...
    void WorkerLoop() {
        ...
        while (running_) {
            ...
            if (!timeouts_.empty()) {
                auto nextDeadline = std::min_element(
                    timeouts_.begin(), timeouts_.end(),
...
public:
    ...
    explicit ConnectionLimits(const Config& config = Config{})
        : config_(config) {
    }
...
private:
    void CleanOldRecords(const std::chrono::steady_clock::time_point& now) {
        ...
        connectionTimes_.erase(
            std::remove_if(connectionTimes_.begin(), connectionTimes_.end(),
