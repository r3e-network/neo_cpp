...
class WorkingNeoNode
{
  ...
  private:
    void InitializeNativeContracts()
    {
        ...
        try
        {
            ...
            if (contractManagement)
            {
                ...
                LOG_INFO("  ✓ ContractManagement - Initialized (Hash: {})",
                         contractManagement->GetScriptHash().ToString());
            }
            else
            {
    ...
    void DisplayNodeInfo()
    {
        ...
        std::cout << "║   • Current Height: " << block_height_
                  << std::string(39 - std::to_string(block_height_).length(), ' ') << "║\n";
        std::cout << "║   • Total Transactions: " << tx_count_
                  << std::string(35 - std::to_string(tx_count_).length(), ' ') << "║\n";
