...
void CLIService::SetupShutdownHandlers() {
    ...
    shutdownManager.RegisterHandler("stop_new_connections", [this]() {
        if (p2p_server_) {
    ...
    shutdownManager.RegisterHandler("close_wallet", [this]() {
        CloseWallet();
    }, 20, std::chrono::seconds(5));
    
    // Priority 30: Stop consensus
    shutdownManager.RegisterHandler("stop_consensus", [this]() {
        if (consensus_) {
            consensus_->Stop();
        }
    ...
    shutdownManager.RegisterHandler("flush_mempool", [this]() {
    ...
    shutdownManager.RegisterHandler("stop_p2p", [this]() {
        if (p2p_server_) {
            p2p_server_->Stop();
        }
    ...
    shutdownManager.RegisterHandler("stop_rpc", [this]() {
        if (rpc_server_) {
            rpc_server_->Stop();
        }
    ...
    shutdownManager.RegisterHandler("stop_monitoring", [this]() {
    ...
    shutdownManager.RegisterHandler("close_storage", [this]() {
        if (store_) {
            store_->Flush();
            store_->Close();
        }
    ...
    shutdownManager.RegisterHandler("final_cleanup", [this]() {
        running_ = false;
...
void CLIService::InitializeMetrics() {
    ...
    auto blockProcessingTime = PROMETHEUS_HISTOGRAM("neo_block_processing_seconds", 
        "Time to process a block", std::vector<double>{0.01, 0.05, 0.1, 0.5, 1, 5});
    ...
    // RPC metrics
    auto rpcRequests = PROMETHEUS_LABELED_COUNTER("neo_rpc_requests_total", 
        "Total RPC requests", {"method"});
    auto rpcLatency = PROMETHEUS_LABELED_HISTOGRAM("neo_rpc_latency_seconds",
        "RPC request latency", {"method"});
    auto rpcErrors = PROMETHEUS_LABELED_COUNTER("neo_rpc_errors_total",
        "Total RPC errors", {"method", "error"});
    ...
    std::thread([this, blockHeight, peerCount, mempoolSize]() {
        while (running_) {
...
void CLIService::InitializeHealthChecks() {
    ...
    auto p2pCheck = std::make_shared<monitoring::P2PHealthCheck>(
        [this]() {
            return p2p_server_ ? p2p_server_->GetConnectedCount() : 0;
        },
        3 // minimum peers
    );
    ...
    public:
        RpcHealthCheck(rpc::RpcServer* server)
            : HealthCheck("rpc"), server_(server) {}
...
void CLIService::StartMonitoring() {
    ...
    limitsConfig.maxConnectionsPerIP = core::Config::GetUInt32("ApplicationConfiguration.P2P.MaxConnectionsPerAddress", 3);
