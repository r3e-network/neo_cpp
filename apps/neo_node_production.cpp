...
public:
    ...
    void Shutdown()
    {
        ...
        if (p2pServer_) p2pServer_->Stop();
        
        // Flush blockchain data
        if (blockchain_) blockchain_->Flush();
...
private:
    ...
    void InitializeP2PNetwork()
    {
        ...
        p2pServer_ = std::make_shared<P2PServer>(
            neoSystem_,
            p2pPort,
            maxConnections
        );
    ...
    void InitializeConsensus()
    {
        ...
        try
        {
            ...
            consensus_ = std::make_shared<DbftConsensus>(
                consensusConfig,
                nodeId,
                validators,
                mempool_,
            ...
            // Set callbacks
            consensus_->SetTransactionVerifier([this](const auto& tx) {
                return blockchain_->VerifyTransaction(tx);
            });
            
            consensus_->SetBlockPersister([this](const auto& block) {
                return blockchain_->Persist(block);
            });
            
            consensus_->SetMessageBroadcaster([this](const auto& msg) {
                p2pServer_->Broadcast(msg);
            });
