...
namespace neo::cli
{
    ...
    void MainService::Start(const CommandLineOptions& options)
    {
        ...
        try
        {
            ...
            if (synchronizer)
            {
                synchronizer->SetStateChangedCallback([this](network::p2p::SynchronizationState state) {
                    switch (state)
                    {
                ...
                synchronizer->SetBlockReceivedCallback([this](const std::shared_ptr<ledger::Block>& block) {
    ...
    void MainService::InitializeWalletCommands()
    {
        ...
        RegisterCommand("showaddress", [this](const std::vector<std::string>& args) {
            OnShowAddress();
            return true;
        }, "Wallet");

        RegisterCommand("transfer", [this](const std::vector<std::string>& args) {
            if (args.size() < 3)
            {
    ...
    void MainService::InitializeTypeConverters()
    {
        ...
        RegisterTypeConverter("string", [](const std::vector<std::string>& args, bool canConsumeAll) -> void* {
            if (args.empty())
                return new std::string("");
            return new std::string(args[0]);
        });

        RegisterTypeConverter("int", [](const std::vector<std::string>& args, bool canConsumeAll) -> void* {
            if (args.empty())
                throw std::runtime_error("Missing argument for int");
            return new int(std::stoi(args[0]));
        });

        RegisterTypeConverter("uint32_t", [](const std::vector<std::string>& args, bool canConsumeAll) -> void* {
            if (args.empty())
    ...
    void MainService::OnShowBalance()
    {
        ...
        try
        {
            ...
            for (const auto& account : accounts)
            {
                ...
                // Get NEO balance
                auto neoBalance = currentWallet_->GetBalance(smartcontract::native::NeoToken::SCRIPT_HASH, account->GetScriptHash());

                // Get GAS balance
                auto gasBalance = currentWallet_->GetBalance(smartcontract::native::GasToken::SCRIPT_HASH, account->GetScriptHash());
