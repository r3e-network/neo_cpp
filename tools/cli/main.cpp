    ...
    if (fieldPos != std::string::npos)
    {
        ...
        if (colonPos != std::string::npos)
        {
            ...
            while (valueEnd < jsonContent.length() && (std::isdigit(jsonContent[valueEnd]) ||
                                                       jsonContent[valueEnd] == '.' || jsonContent[valueEnd] == '-'))
            {
                valueEnd++;
            }
...
std::shared_ptr<Configuration> LoadConfiguration()
{
    ...
    if (configFile.is_open())
    {
        ...
        try
        {
            ...
            if (rpcPortPos != std::string::npos)
            {
                ...
                if (colonPos != std::string::npos)
                {
                    ...
                    if (valueEnd > valueStart)
                    {
                        ...
                        {
                            std::cerr << "Warning: Failed to parse port number '" << portStr << "': " << e.what()
            ...
            ParseConfigurationField(jsonContent, "\"p2pPort\"",
            ...
            ParseConfigurationField(jsonContent, "\"maxConnections\"",
                                    [&](const std::string& value)
                                    {
                                        try
                                        {
            ...
            ParseConfigurationString(jsonContent, "\"dataDirectory\"",
                                     [&](const std::string& value)
                                     {
            ...
            ParseConfigurationString(jsonContent, "\"network\"",
                                     [&](const std::string& value)
                                     {
...
int main(int argc, char* argv[])
{
    ...
    else if (args[0] == "start")
    {
        ...
        try
        {
            ...
            auto protocolSettings =
                std::make_shared<neo::config::ProtocolSettings>(neo::config::ProtocolSettings::GetDefault());
            ...
            try
            {
                ...
                std::cout << "  - Peer discovery: " << (p2pConfig.enable_discovery ? "enabled" : "disabled") <<
                std::endl;
                */
            }
            catch (const std::exception& e)
